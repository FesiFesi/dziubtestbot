...
class DailyRewardsSystem {
    constructor() {
        ...
        console.log("System nagród dziennych: konstruktor", this.currentState);
    }
    ...
    initialize() {
        try {
            ...
            console.log("Inicjalizacja systemu nagród dziennych...");
            ...
            console.log('System nagród dziennych zainicjalizowany:', this.currentState);
        } catch (error) {
    ...
    loadRewardState() {
        ...
        try {
            ...
            console.error('Błąd wczytywania stanu nagród:', error);
        }
    ...
    saveRewardState() {
        try {
            ...
            console.log("Stan nagród zapisany:", this.currentState);
        } catch (error) {
            console.error('Błąd zapisywania stanu nagród:', error);
        }
    }
    ...
    checkStreakReset() {
        ...
        console.log(`Dni od ostatniego odbioru: ${daysDiff}`);
    ...
    startNewWeek() {
        console.log(`Rozpoczynanie nowego tygodnia! Poprzedni: ${this.currentState.weekNumber}`);
        ...
        showNotification(`Gratulacje! Ukończyłeś tydzień ${this.currentState.weekNumber - 1}! Nagrody w nowym tygodniu będą lepsze!`);
    }
    ...
    getNextClaimDay() {
        ...
        if (this.checkStreakReset()) {
            console.log("Resetowanie postępu dniowego, ale zachowanie tygodnia");
    ...
    claimReward() {
        if (!this.canClaimReward()) {
            showNotification('Nie możesz teraz odebrać nagrody. Wróć jutro!');
            return false;
        }
        ...
        if (nextDay > this.rewards.length) {
            showNotification('Wszystkie nagrody zostały już odebrane!');
            return false;
        }
        ...
        if (!reward) {
            showNotification('Błąd: Nie znaleziono nagrody na ten dzień!');
            return false;
        }
    ...
    showRewardNotification(reward) {
        ...
        let message = `Otrzymałeś nagrodę dzienną (tydzień ${weekNumber}): +${displayValue} `;
        
        switch (reward.type) {
            case 'seeds':
                message += finalValue === 1 ? 'ziarenko!' : 'ziarenek!';
                break;
            case 'coins':
                message += finalValue === 1 ? 'DziubCoin!' : 'DziubCoinów!';
                break;
            case 'fruits':
                ...
                if (finalValue === 1) {
                    ...
                    message += 'owocu!';
                } else if (finalValue >= 2 && finalValue < 5) {
                    message += 'owoce!';
                } else {
                    message += 'owoców!';
                }
    ...
    resetRewards() {
        ...
        if (typeof showNotification === 'function') {
            showNotification('Zresetowano nagrody dzienne!');
        } else {
            alert('Zresetowano nagrody dzienne!');
        }
    }
    ...
    setupUI() {
        ...
        weekText.textContent = `Tydzień ${this.currentState.weekNumber}`;
        ...
        streakInfo.textContent = 'Seria trwa! Tak trzymaj!';
        ...
        this.rewards.forEach(reward => {
            ...
            rewardItem.innerHTML = `
                <div class="day-number">Dzień ${reward.day}</div>
        ...
        claimButtonContainer.innerHTML = `
            <button class="claim-daily-reward-button">Odbierz nagrodę</button>
        `;
        ...
        rewardTimer.textContent = 'Następna nagroda za: 00h 00m 00s';
        ...
        try {
            ...
            if (streakInfo) {
                ...
                if (nextDay === 1 && this.currentState.lastClaimDay !== 0) {
                    streakInfo.textContent = 'Seria przerwana! Zacznij od nowa, zachowując postęp tygodnia.';
            ...
            if (claimButton) {
                if (this.canClaimReward()) {
                    claimButton.textContent = 'Odbierz nagrodę';
                    claimButton.disabled = false;
                    claimButton.style.opacity = '1';
                    claimButton.style.cursor = 'pointer';
                } else {
                    claimButton.textContent = 'Wróć jutro po więcej!';
    ...
    updateNextRewardTimer() {
        try {
            ...
            const updateTimer = () => {
                ...
                timerElement.textContent = `Następna nagroda za: ${hours.toString().padStart(2, '0')}h ${minutes.toString().padStart(2, '0')}m ${seconds.toString().padStart(2, '0')}s`;
            ...
            document.querySelectorAll('.daily-reward-item').forEach(item => {
                item.addEventListener('click', () => {
                    ...
                    if (isNext && !isClaimed && this.canClaimReward()) {
                        ...
                        showNotification('Odbierz nagrody po kolei!');
                    } else {
                        showNotification('Nie możesz teraz odebrać nagrody. Wróć jutro!');
                    }
                });
...
setTimeout(function() {
    try {
        if (dailyRewardsSystem && dailyRewardsSystem.canClaimReward()) {
            showNotification('Masz nagrodę do odebrania w kalendarzu!');
