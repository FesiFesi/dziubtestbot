...
function initShop() {
    ...
    // Dodaj obsługę przycisku sklepu
...
function displayActiveBonuses(location) {
    ...
    title.textContent = 'Aktywne bonusy';
    ...
    if (activeBonus.feedCost > 0) {
        ...
        costBonus.textContent = `🌱 -${activeBonus.feedCost}% koszt karmienia`;
    ...
    if (activeBonus.reward > 0) {
        ...
        rewardBonus.textContent = `💰 +${activeBonus.reward}% więcej DziubCoinów`;
    ...
    if (activeBonus.feedTime > 0) {
        ...
        timeBonus.textContent = `⏱️ -${activeBonus.feedTime}% czas karmienia`;
...
function ensureDecorationsExist() {
    ...
    const defaultParkDecorations = {
        bench: { 
            ...
            id: "bench",
            name: "Ławka w parku", 
            description: "Urocza ławka na której siadają goście parku, by karmić dzióbasy",
            price: 0.01, 
        ...
        fountain: { 
            ...
            id: "fountain",
            name: "Fontanna", 
            description: "Piękna fontanna przyciągająca DziubCoiny",
            price: 0.01, 
        ...
        balloon: { 
            ...
            description: "Kolorowy balon, dzięki któremu ptaki szybciej jedzą aby wrócić do zabawy",
            price: 0.05, 
    ...
    const defaultLakeDecorations = {
        boat: { 
            ...
            id: "boat",
            name: "Łódka", 
            description: "Mała łódka, która pozwala podpłynąć bliżej do ptaków, dzięki czemu karmienie jest tańsze",
            price: 0.01, 
        ...
        kite: { 
            ...
            id: "kite",
            name: "Latawiec", 
            description: "Kolorowy latawiec, który pomaga przyciągać ptaki i zwiększa ich hojność",
            price: 0.01, 
        ...
        paraglider: { 
            ...
            id: "paraglider",
            name: "Paralotnia", 
            description: "Widowiskowa paralotnia, dzięki której ptaki są bardziej skupione na jedzeniu",
            price: 0.05, 
        ...
        if (isOwned) {
            ...
            if (decoration === 'balloon') {
                ...
                if (isLocked) {
                    ...
                    requirementInfo.textContent = "Wymaga: Ławka i Fontanna";
            ...
            if (decoration === 'paraglider') {
                ...
                if (isLocked) {
                    const requirementInfo = document.createElement('div');
                    requirementInfo.className = 'decoration-requirement';
                    requirementInfo.textContent = "Wymaga: Łódka i Latawiec";
                    requirementInfo.style.color = "#FF5722";
                    requirementInfo.style.fontStyle = "italic";
                    requirementInfo.style.fontSize = "12px";
                    requirementInfo.style.marginTop = "5px";
...
function buyDecoration(location, decoration) {
    ...
    if (!gameState.decorations?.[location]?.[decoration]) {
        ...
        showNotification("Błąd: Ta dekoracja nie istnieje!");
        return false;
    }
    ...
    if (decorationObj.owned) {
        showNotification("Już posiadasz tę dekorację!");
        return false;
    }
    ...
    if (decoration === 'balloon') {
        ...
        if (!hasBench || !hasFountain) {
            showNotification("Musisz najpierw kupić ławkę i fontannę!");
            return false;
        }
    }
    ...
    if (decoration === 'paraglider') {
        ...
        if (!hasBoat || !hasKite) {
            showNotification("Musisz najpierw kupić łódkę i latawiec!");
            return false;
        }
    }
    ...
    if (gameState.resources.ton < price) {
        showNotification(`Za mało TON! Potrzebujesz ${price.toFixed(3)} TON.`);
        return false;
    }
    ...
    if (bonusType === 'feedCost') {
        bonusText = ` Teraz karmienie ptaków kosztuje o ${decorationObj.bonus.value}% mniej!`;
    } else if (bonusType === 'reward') {
        bonusText = ` Teraz otrzymujesz o ${decorationObj.bonus.value}% więcej DziubCoinów!`;
    } else if (bonusType === 'feedTime') {
        bonusText = ` Teraz ptaki jedzą o ${decorationObj.bonus.value}% szybciej!`;
    }
    
    showNotification(`Zakupiono dekorację: ${decorationObj.name}!${bonusText}`);
    ...
    if (button && typeof showRewardAnimation === 'function') {
        showRewardAnimation(`Nowa dekoracja!`, button);
    }
