...
showRewardAnimation('+1', document.getElementById('collect-button'));
...
function collectAutoSeeds() {
    if (gameState.resources.autoSeedAmount >= 1) {
        ...
        // Pokaż animację
        showRewardAnimation(`+${gameState.resources.autoSeedAmount}`, document.getElementById('collect-auto-button'));
...
window.claimMissionReward = function(missionIndex) {
    ...
    if (mission.completed && mission.rewarded !== true) {
        ...
        if (button) {
            ...
            showRewardAnimation(`+${mission.reward} 💰`, button);
        } else {
...
// Funkcja bonusu dziennego
function claimDailyBonus() {
    const today = new Date().toDateString();
    
    // Sprawdź, czy bonus już został odebrany dzisiaj
    if (gameState.lastDailyBonus === today) {
        showNotification("Już odebrałeś dzisiejszy bonus!");
        return false;
    }
    
    // Przyznaj bonus
    const coinBonus = 5 + Math.floor(gameState.player.level / 2);
    const seedBonus = 10 + gameState.player.level;
    
    gameState.resources.coins += coinBonus;
    gameState.resources.seeds += seedBonus;
    
    // Dodaj XP za odebranie bonusu
    addExperience(10);
    
    // Zapisz datę odebrania bonusu
    gameState.lastDailyBonus = today;
    
    // Pokaż powiadomienie
    showNotification(`Otrzymałeś bonus dzienny: +${coinBonus} DziubCoinów i +${seedBonus} ziarenek!`);
    
    // Pokaż animację nagrody
    showRewardAnimation(`+${coinBonus} 💰`, document.getElementById('daily-bonus-button'));
    
    // Zaktualizuj przycisk bonusu dziennego
    updateDailyBonusButton();
...
function collectAllRewards(locationId) {
    ...
    if (successfulCollects > 0) {
        ...
        if (button) {
            showRewardAnimation(`+${totalCoins} 💰`, button);
        }
    } else {
