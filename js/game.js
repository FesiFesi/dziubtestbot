...
function addSeedManually() {
    console.log("Dodaję ziarenko ręcznie");
...
showRewardAnimation('+1 ziarenek', document.getElementById('collect-button'));
...
function collectAutoSeeds() {
    if (gameState.resources.autoSeedAmount >= 1) {
        ...
        // Pokaż animację
      showRewardAnimation(`+${gameState.resources.autoSeedAmount} ziarenek`, document.getElementById('collect-auto-button'));
        ...
        showNotification("Brak ziarenek do zebrania!");
    }
}
...
function addExperience(amount) {
    ...
    while (gameState.player.xp >= gameState.player.nextLevelXp) {
        ...
        showNotification(`Osiągnięto poziom ${gameState.player.level}! +${10 * gameState.player.level} DziubCoinów!`);
...
function feedBird(slotIndex, locationId) {
    ...
    if (slot.birdType === 'mythical') {
        ...
        showNotification("Za mało ziarenek!");
        return false;
    }
}
...
function unlockBirdSlot(slotIndex, locationId) {
    ...
    if (gameState.resources.coins >= unlockCost) {
        ...
        // Pokaż powiadomienie
        showNotification("Odblokowano nowy slot na ptaki!");
    ...
    showNotification("Za mało DziubCoinów!");
    return false;
}
    ...
    switch (upgradeType) {
        case 'auto-capacity':
            ...
            if (gameState.resources.coins >= cost) {
                ...
                description = "Zwiększono pojemność Auto Zbierusia!";
                ...
                if (capacityButton) {
                    ...
                    capacityButton.textContent = `Ulepsz (${newCost} DziubCoinów)`;
                }
            } else {
                showNotification("Za mało DziubCoinów!");
                return false;
            ...
            if (slot.isActive && slot.isFeeding) {
                ...
                if (slot.remainingTime <= 0) {
                    ...
                    if (isCurrentLocation) {
                        ...
                        showNotification("Ptak gotowy do odbioru nagrody!");
    ...
    gameState.missions.forEach(mission => {
        if (mission.type === missionType && !mission.completed) {
            ...
            if (mission.progress >= mission.target) {
                ...
                // Powiadom gracza
                showNotification(`Misja ukończona: ${mission.title}! Kliknij "Odbierz", aby odebrać nagrodę.`);
...
window.claimMissionReward = function(missionIndex) {
    ...
    // NAPRAWNE: dodajemy event.preventDefault()
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }
    
    // NAPRAWNE: konwersja na liczbę
    ...
    if (mission.completed && mission.rewarded !== true) {
        ...
        // Powiadom gracza
        showNotification(`Odebrano nagrodę za misję: ${mission.reward} DziubCoinów!`);
        
        // Spróbuj pokazać animację, jeśli element istnieje
        ...
        if (button) {
         ...
         showRewardAnimation(`+${totalCoins} DziubCoinów`, button);
        } else {
            console.warn("Nie znaleziono przycisku dla animacji");
        }
        ...
        if (typeof updateMissionsUI === 'function') {
            ...
            console.warn("Brak funkcji updateMissionsUI");
            // Alternatywnie używamy setupMissions
            if (typeof setupMissions === 'function') {
                setupMissions();
            }
        }
        
        // Aktualizuj znacznik misji, po odebraniu nagrody
        updateMissionBadge();
...
function updateBonusMission() {
    ...
    if (bonusMission.progress >= bonusMission.target && !bonusMission.completed) {
        ...
        showNotification("Misja bonusowa ukończona! Odbierz specjalną nagrodę!");
...
function initDefaultMissions() {
    ...
    const newMissions = [
        { 
            type: 'collectSeeds', 
            title: `Zbierz ${30 + playerLevel * 10} ziarenek`, 
            progress: 0, 
        ...
        { 
            type: 'feedBirds', 
            title: `Nakarm ${2 + Math.floor(playerLevel/2)} ptaki`, 
            progress: 0, 
        ...
        { 
            type: 'scareBirds', 
            title: `Przepędź ${3 + Math.floor(playerLevel/2)} ptaków`, 
            progress: 0, 
        ...
        { 
            type: 'autoCollect', 
            title: `Zbierz ${5 + playerLevel} razy auto-zbiór`, 
            progress: 0, 
        ...
        { 
            type: 'dailyBonus', 
            title: `Wykonaj wszystkie dzisiejsze misje`, 
            progress: 0, 
            target: 4, 
    ...
    // Pokazaj powiadomienie
    if (typeof showNotification === 'function') {
        showNotification("Nowe misje są dostępne!");
    }
...
function showRewardAnimation(text, element) {
    ...
    if (parts) {
        ...
        if (type === '💰' || type.includes('DziubCoin') || type.includes('monet')) {
            // DziubCoiny
            iconHtml = `<img src="assets/images/coin-icon.png" style="width: 24px; height: 24px; vertical-align: middle; margin-left: 5px;">`;
        } else if (type.includes('ziarenko') || type.includes('ziarenek')) {
            // Ziarenka
            iconHtml = `<img src="assets/images/seed-icon.png" style="width: 24px; height: 24px; vertical-align: middle; margin-left: 5px;">`;
        } else if (type.includes('owoc') || type.includes('owoców')) {
            // Owoce
            iconHtml = `<img src="assets/images/fruit-icon.png" style="width: 24px; height: 24px; vertical-align: middle; margin-left: 5px;">`;
        } else if (type.includes('TON')) {
            // TON
            iconHtml = `<img src="assets/images/ton-icon.png" style="width: 24px; height: 24px; vertical-align: middle; margin-left: 5px;">`;
        } else {
...
function initDefaultWeeklyMissions() {
    ...
    if (shouldResetMissions) {
        ...
        const newWeeklyMissions = [
            ...
            { 
                type: 'feedEpicBirds', 
                title: `Nakarm ${3 + Math.floor(playerLevel/2)} epickich ptaków`, 
                description: `Znajdź i nakarm ${3 + Math.floor(playerLevel/2)} epickich ptaków`,
                progress: 0, 
                target: 3 + Math.floor(playerLevel/2), 
            ...
            { 
                ...
                title: `Zbierz ${50 + playerLevel * 5} DziubCoinów`, 
                description: `Zbierz w sumie ${50 + playerLevel * 5} DziubCoinów`,
                progress: 0, 
        ...
        if (typeof showNotification === 'function') {
            showNotification("Nowe misje tygodniowe są dostępne!");
        }
    }
}
...
function updateWeeklyMissionProgress(missionType, amount) {
    ...
    gameState.weeklyMissions.forEach(mission => {
        if (mission.type === missionType && !mission.completed) {
            ...
            if (mission.progress >= mission.target) {
                ...
                // Powiadom gracza
                showNotification(`Misja tygodniowa ukończona: ${mission.title}! Kliknij "Odbierz", aby odebrać nagrodę.`);
    ...
    // Aktualizuj badge informujący o nagrodach do odebrania
    updateMissionBadge();
...
window.claimWeeklyMissionReward = function(missionIndex) {
    ...
    if (mission.completed && mission.rewarded !== true) {
        ...
        // Powiadom gracza
        showNotification(`Odebrano nagrodę za misję tygodniową: ${mission.reward} DziubCoinów!`);
...
function feedAllBirds(locationId) {
    ...
    if (requiredSeeds > gameState.resources.seeds) {
        showNotification(`Nie masz wystarczająco ziarenek! Potrzebujesz ${requiredSeeds} ziarenek.`);
        return;
    }
    ...
    if (successfulFeeds > 0) {
        showNotification(`Nakarmiono ${successfulFeeds} ptaków w lokacji ${gameState.locations.configs[locationId].name}! Zużyto ${seedsUsed} ziarenek.`);
    } else {
        showNotification(`Brak ptaków do nakarmienia w lokacji ${gameState.locations.configs[locationId].name}!`);
    }
...
function collectAllRewards(locationId) {
    ...
    if (successfulCollects > 0) {
        showNotification(`Odebrano nagrody od ${successfulCollects} ptaków: +${totalCoins} DziubCoinów!`);
     ...
     showRewardAnimation(`+${totalCoins} DziubCoinów`, button);
        ...
        showNotification("Brak nagród do odebrania!");
    }
...
window.forceFeedDiscount = function() {
    ...
    showNotification("Aktywowano wymuszony bonus -10% za ławkę!");
    console.log("Funkcja karmienia nadpisana z wymuszonym bonusem -10%");
    
    return "Aktywowano wymuszony bonus -10% za ławkę!";
};
    ...
    missionItems.forEach(item => {
        ...
        const newHTML = `
            <div class="mission-header">
                ...
                ${item.classList.contains('mission-complete') ? '<div class="mission-badge">Ukończono</div>' : 
                  item.classList.contains('mission-rewarded') ? '<div class="mission-badge">Odebrano</div>' : 
                  item.classList.contains('mission-bonus') ? '<div class="mission-badge">Bonus</div>' : ''}
            </div>
