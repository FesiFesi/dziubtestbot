...
function showSeedAnimation(buttonElement) {
    ...
    // Animuj w górę i zanikaj
    setTimeout(() => {
        animation.style.transform = 'translateY(-100px)';
        animation.style.opacity = '0';
    }, 50);
    
    // Usuń po dokładnie takim samym czasie jak w showRewardAnimation
    setTimeout(() => {
...
function addSeedManually() {
 ...
 // Użyj nowej animacji z ikoną ziarenka
...
slot.feedStartTime = now;
slot.originalFeedTime = feedTime;
slot.feedEndTime = now + (feedTime * 1000); // Dodaj punkt końcowy
...
function unlockBirdSlot(slotIndex, locationId) {
    ...
    // 🟢 NAPRAWKA: Pobierz konfigurację kosztów dla danej lokacji
    const locationConfig = gameState.locations.configs[locationId];
    const slotUnlockCosts = locationConfig.slotUnlockCosts || [0, 0, 50, 200, 500];
    
    // 🟢 NAPRAWKA: Koszt odblokowania slotu z konfiguracji lokacji
...
function synchronizeTimers() {
    console.log("Synchronizacja timerów UI");
    
    const currentTime = Date.now();
    
    // Aktualizuj tylko sloty w bieżącej lokacji
    ...
    if (gameState.locationSlots[currentLocation]) {
        gameState.locationSlots[currentLocation].forEach((slot, index) => {
            if (slot.isActive && slot.isFeeding) {
                // Użyj tego samego systemu co w updateGameState
                if (slot.feedEndTime) {
                ...
                console.log(`Synchronizacja slot ${index}: remaining=${slot.remainingTime}s`);
...
Object.keys(gameState.locationSlots).forEach(locationId => {
    ...
    gameState.locationSlots[locationId].forEach((slot, index) => {
        if (slot.isActive && slot.isFeeding) {
            // Sprawdź ile czasu faktycznie minęło





            // Zawsze używaj feedEndTime dla konsystentności
if (slot.feedEndTime) {
    const timeRemaining = Math.floor((slot.feedEndTime - currentTime) / 1000);
    slot.remainingTime = Math.max(0, timeRemaining);
} else if (slot.feedStartTime && slot.originalFeedTime) {
    // Napraw brakujący feedEndTime
    slot.feedEndTime = slot.feedStartTime + (slot.originalFeedTime * 1000);
    const timeRemaining = Math.floor((slot.feedEndTime - currentTime) / 1000);
    slot.remainingTime = Math.max(0, timeRemaining);
}

// Dodatkowe zabezpieczenie - jeśli nadal brak danych, ustaw remainingTime na 0
if (!slot.feedEndTime && !slot.feedStartTime) {
    slot.remainingTime = 0;
}
...
function loadGame() {
    ...
    if (savedGame) {
        try {
            ...
            // NAPRAWIONE: Najpierw upewnij się, że mamy podstawową strukturę
            if (!window.gameState) {
                window.gameState = {};
            }
            
            // NAPRAWIONE: Zawsze inicjalizuj pełną konfigurację lokacji PRZED migracją
            initializeLocationConfigs();
            
            // Przeprowadź migrację
            const migratedState = migrateGameState(loadedState);
            
            // Skopiuj dane z migrowanego stanu
            Object.keys(migratedState).forEach(key => {
                if (key !== 'locations') { // locations już zainicjalizowane
                    window.gameState[key] = migratedState[key];
                }
            });
            
            // Skopiuj dane lokacji z zachowaniem już zainicjalizowanej konfiguracji
            if (migratedState.locations) {
                if (migratedState.locations.currentLocation) {
                    window.gameState.locations.currentLocation = migratedState.locations.currentLocation;
                }
                if (migratedState.locations.unlockedLocations) {
                    window.gameState.locations.unlockedLocations = {...migratedState.locations.unlockedLocations};
                }
            }
            
            // Reszta kodu wczytywania...
            const now = Date.now();
            ...
            // Dla nowego gracza - inicjalizuj wszystko od zera
            initializeLocationConfigs();
            initDefaultMissions();
            initDefaultWeeklyMissions();
        }
    } else {
        console.log("Nowy gracz - inicjalizuję wszystko od zera");
        // NAPRAWIONE: Dla nowego gracza zawsze inicjalizuj pełną konfigurację
        initializeLocationConfigs();
        initDefaultMissions();
        initDefaultWeeklyMissions();
    }
    
    // Upewnij się, że wszystkie struktury są zainicjalizowane
...
// NOWA FUNKCJA: Inicjalizacja pełnej konfiguracji lokacji
function initializeLocationConfigs() {
    console.log("Inicjalizacja pełnej konfiguracji lokacji");
    ...
    // Zawsze ustaw pełną konfigurację
    window.gameState.locations.configs = {
        park: {
            name: "Park Miejski",
            background: "assets/images/feed-bg.jpg",
            unlockCost: 0,
        ...
        forest: {
            ...
            background: "assets/images/forest-bg.jpg",
            unlockCost: 1000,
            slotUnlockCosts: [0, 0, 100, 350, 500, 1000],
            birdImages: {
                common: "./assets/images/forest-common.png",
                rare: "./assets/images/forest-rare.png",
                epic: "./assets/images/forest-epic.png",
    ...
    // Ustaw domyślne wartości dla lokacji
    ...
    console.log("Konfiguracja lokacji zainicjalizowana:", window.gameState.locations.configs);
}

// NOWA FUNKCJA: Zapewnienie integralności gameState
function ensureGameStateIntegrity() {
    console.log("Sprawdzanie integralności gameState");
    
    // Upewnij się, że wszystkie struktury istnieją
    ...
    // Dla każdej lokacji w configs, upewnij się że mamy odpowiednie struktury
    Object.keys(gameState.locations.configs).forEach(locationId => {
        // Sloty
        if (!gameState.locationSlots[locationId]) {
            gameState.locationSlots[locationId] = [
                {isActive: false, birdType: null, isFeeding: false, needsCollection: false, remainingTime: 0},
                {isActive: false, birdType: null, isFeeding: false, needsCollection: false, remainingTime: 0},
                {isActive: false, birdType: null, isFeeding: false, needsCollection: false, remainingTime: 0},
                {isActive: false, birdType: null, isFeeding: false, needsCollection: false, remainingTime: 0},
        ...
        // Odblokowane sloty
        if (!gameState.locationUnlockedSlots[locationId]) {
            gameState.locationUnlockedSlots[locationId] = [true, true, false, false, false];
        }
        ...
        if (!gameState.discoveredBirds[locationId]) {
            ...
            // Pierwszy ptak w parku odkryty
            if (locationId === "park") {
                gameState.discoveredBirds[locationId]["common_sparrow"] = true;
            }
        }
    });
    
    console.log("Integralność gameState zapewniona");
}
...
setTimeout(() => {
   ...
   // Uruchom aktualizację stanu gry co sekundę z zabezpieczeniem przed wielokrotnym uruchomieniem
if (!window.gameUpdateInterval) {
    window.gameUpdateInterval = setInterval(updateGameState, 1000);
}


// Dodatkowa synchronizacja timerów co 5 sekund dla pewności
setInterval(() => {
...
function initDefaultWeeklyMissions() {
    ...
    // NOWE: Sprawdź czy istnieją stare misje typu "unlockSlots" i wymuś reset
    const hasOldUnlockMission = gameState.weeklyMissions.some(mission => mission.type === 'unlockSlots');
    if (hasOldUnlockMission) {
        console.log("Wykryto starą misję 'unlockSlots' - wymuszam reset misji tygodniowych");
        gameState.weeklyMissions = [];
        gameState.lastWeeklyCheck = null; // Resetuj datę, aby wymusić tworzenie nowych misji
    }
...
// System kompensacji czasu offline przy przełączaniu kart
let lastUpdateTime = Date.now();

// Page Visibility API - wykrywa gdy karta staje się aktywna
document.addEventListener('visibilitychange', function() {
    if (!document.hidden) {
        // Karta stała się aktywna - sprawdź ile czasu minęło
        const currentTime = Date.now();
        const timeOffline = Math.floor((currentTime - lastUpdateTime) / 1000);
        
        if (timeOffline > 2) { // Jeśli offline dłużej niż 2 sekundy
...
// Aktualizuj wszystkie karmiące się ptaki
Object.keys(gameState.locationSlots).forEach(locationId => {
    gameState.locationSlots[locationId].forEach((slot, index) => {
        if (slot.isActive && slot.isFeeding) {
            // Użyj konsystentnego systemu feedEndTime
            if (slot.feedEndTime) {
                const timeRemaining = Math.floor((slot.feedEndTime - currentTime) / 1000);
                slot.remainingTime = Math.max(0, timeRemaining);
            } else if (slot.feedStartTime && slot.originalFeedTime) {
...
// Funkcja aktualizująca UI pojedynczego slotu z ptakiem
function updateBirdSlotUI(slotElement, slotIndex, locationId) {
    ...
    if (slot.isActive) {
        ...
        if (slot.isFeeding) {
    // Ptak się karmi - pokaż timer
    if (birdTimer) {
        // Lepsze zabezpieczenie przed błędnymi wartościami
        let timeToShow = 0;
        if (typeof slot.remainingTime === 'number' && !isNaN(slot.remainingTime) && slot.remainingTime >= 0) {
            timeToShow = Math.floor(slot.remainingTime);
        }
        birdTimer.textContent = formatTime(timeToShow);
        birdTimer.style.display = 'block';
    }
            
            // Ukryj przyciski karmienia i przepędzania
            ...
            // Pokaż przyciski karmienia i przepędzania
...
// Aktualizuj czas referencyjny co sekundę
setInterval(() => {
...
window.initializeLocationConfigs = initializeLocationConfigs;
window.ensureGameStateIntegrity = ensureGameStateIntegrity;
