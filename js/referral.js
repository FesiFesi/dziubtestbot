...
(function() {
    console.log(t('referral.initialization'));
    ...
    const REFERRAL_REWARDS = {
        ...
        tiers: {
            1: { 
                ...
                ton: 0,
                special: null,
                description: t('referral.tiers.tier1'),
                icon: "🥇"
            },
            3: { 
                coins: 50, 
                ...
                ton: 0,
                special: null,
                description: t('referral.tiers.tier3'),
                icon: "🏆"
            },
            5: { 
                coins: 75, 
                ...
                ton: 0,
                special: null,
                description: t('referral.tiers.tier5'),
                icon: "🎖️"
            },
            10: { 
                coins: 100, 
                ...
                ton: 0.01,
                special: null,
                description: t('referral.tiers.tier10'),
                icon: "💰"
            },
            20: { 
                coins: 300, 
                ...
                ton: 0.05,
                special: null,
                description: t('referral.tiers.tier20'),
                icon: "👑"
            },
            50: { 
                coins: 500, 
                ...
                ton: 0.1,
                special: null,
                description: t('referral.tiers.tier50'),
                icon: "💎"
            },
            100: { 
                coins: 700, 
                ...
                description: t('referral.tiers.tier100'),
                icon: "🏅"
            }
        },
        weeklyRewards: {
            5: { coins: 100, fruits: 1, description: t('referral.weekly.tier5') },
            10: { coins: 200, fruits: 2, ton: 0.02, description: t('referral.weekly.tier10') },
            20: { coins: 300, fruits: 3, ton: 0.05, description: t('referral.weekly.tier20') }
        }
    };
            ...
            if (window.Telegram && window.Telegram.WebApp) {
                ...
                if (startParam && startParam.startsWith('ref_')) {
                    ...
                    if (referrerCode === gameState.referral.myReferralCode) {
                        console.log(t('referral.selfInviteAttempt'));
                        return;
                    }
    ...
    function processReferral(referrerCode) {
        console.log(t('referral.processing', { code: referrerCode }));
        ...
        showReferralAnimation(t('referral.welcomeBonus', { amount: REFERRAL_REWARDS.invited }), 'welcome');
    ...
    function copyReferralLink() {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(referralLink).then(function() {
                showReferralAnimation(t('referral.linkCopied'), 'success');
                // Efekt wizualny kopiowania
                animateCopyButton();
            }).catch(function(err) {
                console.error('Błąd kopiowania:', err);
                showNotification(t('referral.copyError'));
            });
        } else {
            ...
            showReferralAnimation(t('referral.linkCopiedSimple'), 'success');
            animateCopyButton();
        }
    }
    ...
    function shareReferralLink() {
        ...
        try {
            ...
            console.error(t('referral.shareError', { error: error }));
            copyReferralLink(); // Fallback
        }
    }
    ...
    function addInvitedUser(userId) {
        if (!gameState.referral.invitedUsers.includes(userId)) {
            ...
            let message = t('referral.newUserJoined', { amount: coinsReward });
            if (gameState.referral.combo > 1) {
                message += ' ' + t('referral.comboMultiplier', { combo: gameState.referral.combo });
            }
    ...
    function checkWeeklyRewards() {
        ...
        Object.entries(REFERRAL_REWARDS.weeklyRewards).forEach(([threshold, reward]) => {
            ...
            if (weeklyInvites >= thresholdNum) {
                ...
                showReferralAnimation(t('referral.weeklyReward', { description: reward.description }), 'weekly-reward');
            }
        });
    }
    ...
    function showTierRewardAnimation(reward, tier) {
        ...
        // Przetłumaczone stringi
        const levelText = t('referral.tierLevel', { level: tier });
        const coinsText = t('resources.coins');
        const fruitsText = reward.fruits > 0 ? (reward.fruits === 1 ? t('resources.fruits', { count: 1 }) : t('resources.fruits', { count: reward.fruits })) : '';
        const tonText = reward.ton > 0 ? t('resources.ton') : '';
        ...
        content.innerHTML = `
            ...
            <h2 style="color: #fff; margin-bottom: 15px; text-shadow: 2px 2px 4px rgba(0,0,0,0.3);">
                ${t('referral.tierLevel', { level: tier })}
            </h2>
            ...
            <div style="background: rgba(255,255,255,0.2); border-radius: 15px; padding: 15px; margin-bottom: 20px;">
                <div style="color: #fff; font-weight: bold; margin-bottom: 10px;">${t('referral.rewards')}:</div>
    ...
    function updateFriendsModal() {
        ...
        friendsBody.innerHTML = `
            <div class="referral-container" style="padding: 15px;">
                <h3 style="color: #2196F3; margin-bottom: 20px; text-align: center;">${t('friends.referralTitle')}</h3>
                ...
                <div class="referral-stats-main" style="background: linear-gradient(135deg, #2196F3, #1976D2); border-radius: 15px; padding: 20px; margin-bottom: 20px; color: white; position: relative; overflow: hidden;">
                    ...
                    <div style="position: relative; z-index: 1;">
                        <div style="text-align: center; font-size: 36px; font-weight: bold; margin-bottom: 10px;">
                            ${totalInvites}
                            ${combo > 1 ? `<span style="font-size: 18px; color: #FFD700;">${t('referral.comboMultiplier', { combo: combo })}</span>` : ''}
                        </div>
                        <div style="text-align: center; font-size: 18px;">${t('friends.invitedFriends')}</div>
                        <div style="display: flex; justify-content: center; margin-top: 20px;">
                            <div style="text-align: center;">
                                <div style="font-size: 24px; font-weight: bold;">${weeklyInvites}</div>
                                <div style="font-size: 14px; opacity: 0.9;">${t('friends.weeklyInvites')}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="referral-link-section" style="background: #f5f5f5; border-radius: 15px; padding: 20px; margin-bottom: 20px;">
                    <p style="margin-bottom: 15px; text-align: center; font-weight: bold; color: #333;">${t('friends.yourReferralLink')}</p>
                    <div class="referral-link-box" style="background: white; border: 2px solid #2196F3; border-radius: 10px; padding: 12px; word-break: break-all; font-size: 14px; margin-bottom: 15px; position: relative;">
                        ${referralLink}
                        <span class="copy-tooltip" style="position: absolute; top: -30px; right: 10px; background: #333; color: white; padding: 5px 10px; border-radius: 5px; font-size: 12px; display: none;">${t('referral.copied')}</span>
                    </div>
                ...
                <div class="referral-tiers" style="background: #fff3e0; border-radius: 15px; padding: 20px; margin-bottom: 20px;">
                    <h4 style="margin-bottom: 20px; color: #f57c00; text-align: center;">${t('referral.tierRewards')}</h4>
                    ${generateTierList()}
                </div>
                
                <!-- Nagrody tygodniowe -->
                <div class="weekly-rewards" style="background: #e8f5e9; border-radius: 15px; padding: 20px; margin-bottom: 20px;">
                    <h4 style="margin-bottom: 15px; color: #4CAF50; text-align: center;">${t('referral.weeklyRewards')}</h4>
        ...
        Object.entries(REFERRAL_REWARDS.tiers).forEach(([tier, reward]) => {
            ...
            html += `
                ...
                ">
                    ...
                    <div style="flex: 1;">
                        <strong style="color: #333; font-size: 16px;">${tierNum} ${tierNum === 1 ? t('referral.invitation', { count: 1 }) : t('referral.invitation', { count: tierNum })}</strong>
                        <div style="color: #666; font-size: 14px; margin-top: 5px;">
                            ${reward.coins} ${t('resources.coins')}
                            ${reward.fruits > 0 ? ` + ${reward.fruits} ${reward.fruits === 1 ? t('resources.fruits', { count: 1 }) : t('resources.fruits', { count: reward.fruits })}` : ''}
                            ${reward.ton > 0 ? ` + ${reward.ton} ${t('resources.ton')}` : ''}
                            ${reward.special ? ` + ${t('referral.' + reward.special.replace(/_/g, ''))}` : ''}
                        </div>
                        ${isNext ? `<div style="color: #FFA500; font-size: 12px; margin-top: 5px;">${t('referral.remaining', { remaining: tierNum - gameState.referral.totalInvites })}</div>` : ''}
                    </div>
            ...
            html += `
                <div style="margin-bottom: 15px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                        <span style="font-weight: bold; color: #333;">${thresholdNum} ${t('referral.invitation', { count: thresholdNum })}</span>
    ...
    function generateInvitedList() {
        ...
        let html = `
            <div style="margin-top: 20px; background: #f5f5f5; border-radius: 15px; padding: 20px;">
                <h4 style="margin-bottom: 15px; color: #1565c0;">${t('friends.invitedUsers')}</h4>
            ...
            html += `
                ...
                ">
                    <div style="display: flex; align-items: center;">
                        ...
                        <div>
                            <div style="font-weight: bold; color: #333;">${t('friends.userPrefix')}${index + 1}</div>
                        </div>
                    </div>
                </div>
            `;
    ...
    function generateSpecialRewards() {
        ...
        let html = `
            <div style="margin-top: 20px; background: linear-gradient(135deg, #f3e5f5, #e1bee7); border-radius: 15px; padding: 20px;">
                <h4 style="margin-bottom: 15px; color: #8E24AA;">${t('friends.specialRewards')}</h4>
        ...
        gameState.referral.specialSkins.forEach(skin => {
            html += `
                ...
                ">
                    ...
                    <span style="font-weight: bold; color: white;">${t('referral.' + skin.replace(/_/g, ''))}</span>
                </div>
            `;
        });
...
// Funkcja pomocnicza do sprawdzania odbiorcy nagrody (wewnętrzna, niewidoczna dla użytkownika)
function prepareForRecruiterSkin() {
    // Sprawdź, czy obiekt skins istnieje
    if (!gameState.skins) {
        gameState.skins = {
            currentSkin: "default",
...
window.claimTierReward = function(tier) {
    ...
    const tierKey = `tier${tier}`;
    if (gameState.referral.rewardsClaimed[tierKey]) {
        console.log(`Nagroda za poziom ${tier} została już odebrana`);
        return;
    }
    ...
    if (reward.coins > 0) {
        gameState.resources.coins += reward.coins;
        console.log(`Dodano ${reward.coins} monet`);
    }
    ...
    if (reward.special && reward.special.startsWith('skin_')) {
        ...
        if (!gameState.referral.specialSkins.includes(reward.special)) {
            gameState.referral.specialSkins.push(reward.special);
            console.log(`Dodano specjalną nagrodę: ${reward.special}`);
        }
        
        // Dodatkowo odblokuj skin w gameState.skins
        if (reward.special === 'skin_recruiter' && gameState.skins) {
            if (!gameState.skins.unlockedSkins.includes('master')) {
                gameState.skins.unlockedSkins.push('master');
                console.log('Odblokowano skin Master Recruiter');
            }

// NOWE: Wyzwól zdarzenie odblokowania skina
document.dispatchEvent(new Event('skinUnlocked'));

// NOWE: Wywołaj odświeżenie systemu skinów, jeśli funkcja istnieje
            if (window.WardrobeModule && typeof window.WardrobeModule.refreshSkinSystem === 'function') {
                window.WardrobeModule.refreshSkinSystem();
            }
            
            // NOWE: Bezpośrednie odświeżenie interfejsu szafy
            if (window.forceUpdateSkinImages) {
                window.forceUpdateSkinImages();




// NOWE: Specjalne powiadomienie o odblokowaniu skina
const currentLang = localStorage.getItem('gameLanguage') || 'pl';
const message = currentLang === 'pl' ? 
    "Odblokowano skin Master Recruiter! Sprawdź Szafę!" :
    "Master Recruiter skin unlocked! Check your Wardrobe!";
...
function prepareForRecruiterSkin() {
    ...
    gameState.referral.rewardsClaimed[tierKey] = true;
    ...
    const currentLang = localStorage.getItem('gameLanguage') || 'pl';
    if (currentLang === 'pl') {
        showNotification(`Odebrano nagrodę za ${tier} ${tier === 1 ? 'zaproszenie' : 'zaproszenia'}!`);
    } else {
        showNotification(`Reward for ${tier} ${tier === 1 ? 'invitation' : 'invitations'} claimed!`);
    }
    ...
    // Dodatkowy eksport funkcji
window.claimTierReward = window.ReferralSystem.claimTierReward;