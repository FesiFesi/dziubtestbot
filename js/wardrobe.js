...
// Napraw odblokowanie skina Leśny Zwiadowca
console.log("Sprawdzanie i bezpośrednia naprawa skina Leśny Zwiadowca");

// Sprawdź czy skin powinien być odblokowany
const forestSkinUnlockedInStorage = localStorage.getItem('forestSkinUnlocked') === 'true';
const hasMetDistanceRequirement = gameState.rewardPass && gameState.rewardPass.distance >= 1100;
const hasClaimedReward = gameState.rewardPass && gameState.rewardPass.claimedRewards && 
                         gameState.rewardPass.claimedRewards.free && 
                         gameState.rewardPass.claimedRewards.free.includes(1100);

if (forestSkinUnlockedInStorage || hasMetDistanceRequirement || hasClaimedReward) {
    console.log("Warunki odblokowania skina Leśny Zwiadowca spełnione - naprawiam");
    ...
    if (!gameState.skins) {
        ...
        if (!Array.isArray(gameState.skins.unlockedSkins)) {
            gameState.skins.unlockedSkins = ["default", "lesny"];
        } else if (!gameState.skins.unlockedSkins.includes("lesny")) {
            gameState.skins.unlockedSkins.push("lesny");
        }
    }
...
// Bezpośrednia naprawa odblokowania skina Leśny Zwiadowca
console.log("Sprawdzanie i bezpośrednia naprawa skina Leśny Zwiadowca");
if ((gameState.rewardPass && gameState.rewardPass.distance >= 1100) || 
   (gameState.rewardPass && gameState.rewardPass.claimedRewards && 
    gameState.rewardPass.claimedRewards.free && 
    gameState.rewardPass.claimedRewards.free.includes(1100))) {
    
    // Bezwzględnie upewnij się, że skin jest odblokowany
    let isUnlocked = gameState.skins.unlockedSkins.includes("lesny");
    if (!isUnlocked) {
        console.log("Naprawianie skina Leśny Zwiadowca - dodawanie do odblokowanych skinów");
        gameState.skins.unlockedSkins.push("lesny");
        
        // Pokaż komunikat
        showNotification("Naprawiono odblokowanie skina Leśny Zwiadowca!");
        saveGame();
...
// Wywołaj funkcję odblokowującą skin Leśny Zwiadowca
this.forceUnlockForestSkin();
...
// Bezpośrednia naprawa odblokowania skina Master Recruiter
console.log("Sprawdzanie i bezpośrednia naprawa skina Master Recruiter");
if (gameState.referral && gameState.referral.totalInvites >= 100) {
    // Bezwzględnie upewnij się, że skin jest odblokowany
    let isUnlocked = gameState.skins.unlockedSkins.includes("master");
    if (!isUnlocked) {
        console.log("Naprawianie skina Master Recruiter - dodawanie do odblokowanych skinów");
        gameState.skins.unlockedSkins.push("master");
        
        // Dodaj również do specialSkins w systemie poleceń
        if (!gameState.referral.specialSkins) {
            gameState.referral.specialSkins = [];
        }
        if (!gameState.referral.specialSkins.includes("skin_recruiter")) {
            gameState.referral.specialSkins.push("skin_recruiter");
        }
        
        // Oznacz nagrodę jako odebraną
        if (!gameState.referral.rewardsClaimed) {
            gameState.referral.rewardsClaimed = {};
        }
        gameState.referral.rewardsClaimed.tier100 = true;
        
        // Pokaż komunikat
        showNotification("Naprawiono odblokowanie skina Master Recruiter!");
        saveGame();
        ...
        // NOWE: Pełne odświeżenie systemu skinów
this.refreshSkinSystem();
...
/* Dodane nowe style dla obrazków skinów */
...
/* Dodatkowa reguła dla skina rekrutera */
...
// Dodaj dodatkową naprawę po inicjalizacji listenerów
if (gameState.referral && gameState.referral.totalInvites >= 100) {
    this.forceUnlockRecruiterSkin();
}
...
// Dodaj tę nową funkcję do modułu WardrobeModule
// Najlepiej dodać pod koniec obiektu WardrobeModule, przed ostatnim nawiasem klamrowym

// Funkcja wymuszająca odblokowanie skina Leśny Zwiadowca
forceUnlockForestSkin: function() {
    console.log("Wymuszenie odblokowania skina Leśny Zwiadowca");
    
    // Sprawdź warunki odblokowania
    ...
    if (gameState.rewardPass && gameState.rewardPass.claimedRewards && 
        gameState.rewardPass.claimedRewards.free && 
        gameState.rewardPass.claimedRewards.free.includes(1100)) {
        shouldUnlock = true;
    }
    ...
    if (shouldUnlock) {
        ...
        // Dodaj skin do odblokowanych, jeśli go tam nie ma
        if (!gameState.skins.unlockedSkins.includes("lesny")) {
            gameState.skins.unlockedSkins.push("lesny");
            console.log("Dodano skin lesny do odblokowanych skinów");
...
setupBackButton: function() {
    ...
    // Dodatkowo dodajmy zabezpieczenie globalne dla przycisku powrotu
    window.addEventListener('click', function(e) {
        if (e.target.id === 'wardrobe-back-button' || e.target.classList.contains('back-button')) {
            const wardrobeScreen = document.getElementById('wardrobe-screen');
            if (wardrobeScreen && wardrobeScreen.classList.contains('active')) {
                console.log("Globalne wykrycie przycisku powrotu z szafy");
                self.hideWardrobeScreen();
            }
        }
    });
...
// NOWA FUNKCJA: Całkowite odświeżenie systemu skinów i interfejsu
    refreshSkinSystem: function() {
        console.log("Odświeżanie systemu skinów po odblokowaniu nowego skina");
        
        // Upewnij się, że mamy strukturę skinów
        if (!gameState.skins) {
            gameState.skins = {
                currentSkin: "default",
                unlockedSkins: ["default"]
        ...
        // Sprawdź, czy skiny specjalne z systemu poleceń są odblokowane
        if (gameState.referral && gameState.referral.specialSkins) {
            gameState.referral.specialSkins.forEach(skinId => {
                if (skinId === 'skin_recruiter' && !gameState.skins.unlockedSkins.includes('master')) {
                    gameState.skins.unlockedSkins.push('master');
                    console.log("Dodano skin rekrutera z systemu poleceń");
                }
            });
        }
...
// Upewnij się, że skin Leśny Zwiadowca jest odblokowany
this.forceUnlockForestSkin();
    ...
    showWardrobeScreen: function() {
        ...
        if (wardrobeScreen) {
            ...
            // DODANE: dodatkowa funkcja do ustawienia filtrów obrazów
setTimeout(() => {
    console.log("Wymuszenie aktualizacji wyglądu obrazków skinów");
    document.querySelectorAll('.skin-item').forEach(item => {
        ...
        const isSelected = gameState.skins.currentSkin === skinId;
        
        // Znajdź obrazek skina bezpośrednio
        const skinImage = item.querySelector('.skin-image img');
        if (skinImage) {
            if (!isUnlocked) {
                // Zablokowany skin - zastosuj szary filtr
                skinImage.style.filter = 'grayscale(100%)';
                skinImage.style.opacity = '0.5';
                console.log(`Zastosowano szary filtr do ${skinId} (wymuszenie)`);
            } else {
                // Odblokowany skin - normalny wygląd
...
// Wymuszenie naprawy skina rekrutera przy każdym otwarciu szafy
if (gameState.referral && gameState.referral.totalInvites >= 100) {
    this.forceUnlockRecruiterSkin();
}
...
updateSkinsUIState: function() {
    ...
    document.querySelectorAll('.skin-item').forEach(item => {
        ...
        // Bezpośrednio znajdujemy obraz skina
        const skinImageContainer = item.querySelector('.skin-image');
        const skinImage = skinImageContainer ? skinImageContainer.querySelector('img') : null;
        ...
        if (skinId === currentSkin) {
            ...
            // Zastosuj szary filtr do obrazka
            if (skinImage) {
                skinImage.style.filter = 'grayscale(100%)';
                skinImage.style.opacity = '0.5';
                console.log(`Zastosowano szary filtr do ${skinId}`);
            } else {
                console.log(`Nie znaleziono obrazka dla ${skinId}`);
            }
...
applySkinToAllScreens: function() {
    ...
    console.log("Ścieżki do grafik:", skinImagePath, skinExpeditionPath);
    
    // 1. Ekran hodowli (breeding)
    const breedingBirdImage = document.getElementById('pet-bird-image');
    if (breedingBirdImage) {
        console.log("Aktualizacja grafiki na ekranie hodowli");
    ...
    if (playBirdImage) {
        console.log("Aktualizacja grafiki na ekranie zabawy");
    ...
    if (rewardPassBird) {
        console.log("Aktualizacja grafiki na ekranie przepustki nagród");
        // POPRAWIONE: Może używać innej ścieżki
        if (skinId === 'default') {
    ...
    if (flyingBird) {
        console.log("Aktualizacja grafiki na ekranie ekspedycji");
    ...
    if (wardrobeBirdImage) {
        console.log("Aktualizacja grafiki na ekranie szafy");
        wardrobeBirdImage.src = skinImagePath;
    }
    
    // 6. Aktualizacja buttonów w szafie
    document.querySelectorAll('.skin-select-button').forEach(button => {
        const buttonSkinId = button.closest('.skin-item')?.getAttribute('data-skin-id');
        if (buttonSkinId === skinId) {
            // Ten skin jest wybrany
            button.textContent = localStorage.getItem('gameLanguage') === 'en' ? 'Selected' : 'Wybrany';
            button.style.backgroundColor = '#4CAF50';
            button.classList.add('selected');
        } else if (!button.disabled) {
            // Inny odblokowany skin
    ...
    // 7. Zaktualizuj stan UI
    if (typeof this.updateBirdPreview === 'function') {
        this.updateBirdPreview();
    }
    
    console.log("Zakończono aktualizację grafik dla skina:", skinId);
},
...
function fixExpeditionButtons() {
    ...
    document.querySelectorAll('button').forEach(button => {
        if (button.textContent === 'activeExpedition.skinUnavailable') {
            ...
            // Ustaw tekst bezpośrednio na podstawie języka
            if (currentLang === 'pl') {
                button.textContent = "Niedostępny";
            } else {
                button.textContent = "Unavailable";
...
document.querySelectorAll('.skin-select-button').forEach(button => {
    if (button.disabled && isExpeditionActive) {
        ...
        if (button.textContent === 'activeExpedition.skinUnavailable' || 
            ...
            // Ustaw tekst bezpośrednio na podstawie aktualnego języka
            const currentLang = localStorage.getItem('gameLanguage') || 'pl';
            if (currentLang === 'pl') {
                button.textContent = "Niedostępny";
            } else {
...
function updateWardrobeTexts() {
    ...
    if (expeditionMessage && isExpeditionActive) {
        if (currentLang === 'pl') {
            ...
            expeditionMessage.textContent = "Your bird is on an expedition! You cannot change skins during an active expedition.";
        }
    }
...
// DODANE: Przechwycenie oryginalnej funkcji loadGame - NAPRAWIONE
...
window.loadGame = function() {
    ...
    setTimeout(function() {
        // Sprawdź dodatkowe dane w localStorage
        // Przywróć currentSkin
        const savedSkin = localStorage.getItem('currentSkin');
        if (savedSkin && window.gameState.skins) {
            window.gameState.skins.currentSkin = savedSkin;
        }
        
        // Sprawdź odblokowane skiny
        const forestSkinUnlocked = localStorage.getItem('forestSkinUnlocked') === 'true';
        if (forestSkinUnlocked && window.gameState.skins && 
            !window.gameState.skins.unlockedSkins.includes('lesny')) {
            window.gameState.skins.unlockedSkins.push('lesny');
        }
        
        // Sprawdź odblokowane lokacje
        const lakeUnlocked = localStorage.getItem('lakeUnlocked') === 'true';
        if (lakeUnlocked && window.gameState.locations) {
            window.gameState.locations.unlockedLocations.lake = true;
        }
        
        if (WardrobeModule && typeof WardrobeModule.recoverSkinState === 'function') {
            WardrobeModule.recoverSkinState();
        }
        ...
        if (typeof fixExpeditionButtons === 'function') {
            fixExpeditionButtons();
        }
        
        if (typeof updateWardrobeTexts === 'function') {
            updateWardrobeTexts();
        }
...
// Uruchom naprawę tekstów po zmianie języka
document.addEventListener('languageChanged', function() {
    // Napraw teksty przycisków
    setTimeout(function() {
        updateWardrobeTexts();
        fixExpeditionButtons();
    }, 100);
...
// Uruchom naprawę tekstów po pełnym załadowaniu strony
window.addEventListener('load', function() {
    setTimeout(function() {
        updateWardrobeTexts();
        fixExpeditionButtons();
...
// Inicjalizacja efektów wizualnych dla skinów
window.addEventListener('load', function() {
    setTimeout(function() {
        console.log("Inicjalizacja efektów wizualnych dla skinów");
        ...
        document.querySelectorAll('.skin-item').forEach(item => {
            ...
            // Sprawdź czy skin jest odblokowany
            const isUnlocked = gameState.skins && 
                               gameState.skins.unlockedSkins && 
                               gameState.skins.unlockedSkins.includes(skinId);
            
            if (!isUnlocked) {
                // Zastosuj efekt szarości dla zablokowanych skinów
                skinImage.style.filter = 'grayscale(100%)';
                skinImage.style.opacity = '0.6';
                item.classList.add('locked');
            } else {
                // Normalny wygląd dla odblokowanych skinów
                skinImage.style.filter = 'none';
                skinImage.style.opacity = '1';
                item.classList.remove('locked');
            }
...
(function() {
    ...
    console.log("Dodano style CSS dla zablokowanych skinów");
})();

// Funkcja wymuszająca aktualizację obrazków skinów
function forceUpdateSkinImages() {
    console.log("Wymuszenie aktualizacji obrazków skinów");
    ...
    document.querySelectorAll('.skin-item').forEach(item => {
        ...
        // Znajdź obrazek bezpośrednio
        const skinImage = item.querySelector('.skin-image img');
        if (!skinImage) return;
        
        if (!isUnlocked) {
            // Zablokowany skin - wymuszony szary filtr
            item.classList.add('locked');
            skinImage.style.filter = 'grayscale(100%)';
            skinImage.style.opacity = '0.5';
            console.log(`Zastosowano szary filtr do ${skinId} (bezpośrednio)`);
        } else {
            // Odblokowany skin - normalny wygląd
            item.classList.remove('locked');
            skinImage.style.filter = 'none';
...
window.addEventListener('load', function() {
    setTimeout(forceUpdateSkinImages, 1000);
    // Uruchom ponownie po dłuższym czasie dla pewności
    setTimeout(forceUpdateSkinImages, 3000);
});

// Uruchom funkcję przy każdej aktywacji ekranu szafy
document.addEventListener('click', function(e) {
    if (e.target.closest('.wardrobe-button')) {
        console.log("Kliknięto przycisk szafy - wymuszam aktualizację obrazków");
        setTimeout(forceUpdateSkinImages, 200);
        setTimeout(forceUpdateSkinImages, 500);
    }
});
...
// Dodaj nową funkcję bezpośredniej naprawy skina rekrutera
WardrobeModule.forceUnlockRecruiterSkin = function() {
    console.log("Wymuszenie odblokowania skina rekrutera");
    ...
    if (!gameState.skins.unlockedSkins.includes("master")) {
        gameState.skins.unlockedSkins.push("master");
        console.log("Dodano skin master do odblokowanych skinów");
    }
    
    // Upewnij się, że system poleceń ma wszystkie potrzebne pola
    if (!gameState.referral) {
        gameState.referral = {
            totalInvites: 100, // Ustawiamy 100 zaproszeń
            specialSkins: []
        };
    }
    
    if (!Array.isArray(gameState.referral.specialSkins)) {
        gameState.referral.specialSkins = [];
    }
    
    // Dodaj skin do specialSkins jeśli go tam nie ma
    if (!gameState.referral.specialSkins.includes("skin_recruiter")) {
        gameState.referral.specialSkins.push("skin_recruiter");
    }
...
this.forceUnlockForestSkin();
    ...
    // Spróbuj również naprawić obrazki skinów
    if (typeof forceUpdateSkinImages === 'function') {
        forceUpdateSkinImages();
    }
    
    // Pokaż komunikat o naprawie
    showNotification("Naprawiono odblokowanie skina Master Recruiter! Odśwież stronę, jeśli nadal widzisz problemy.");
...
// Dodaj funkcję sprawdzającą odblokowanie skina przy każdym załadowaniu strony
window.addEventListener('load', function() {
    // Sprawdź czy skin jest odblokowany w localStorage
    if (localStorage.getItem('forestSkinUnlocked') === 'true') {
        // Upewnij się, że gameState istnieje
        if (!window.gameState) {
            window.gameState = {};
        }
        ...
        if (!window.gameState.skins.unlockedSkins.includes("lesny")) {
            window.gameState.skins.unlockedSkins.push("lesny");
            console.log("Dodano skin lesny do odblokowanych skinów podczas ładowania strony");
    ...
    // Wywołaj funkcję wymuszającą odblokowanie skina
...
// Wywołuj applySkinToAllScreens co 5 sekund przez minutę po załadowaniu
let skinUpdateInterval = setInterval(function() {
    if (WardrobeModule && typeof WardrobeModule.applySkinToAllScreens === 'function') {
        WardrobeModule.applySkinToAllScreens();
    }
}, 5000);
...
setTimeout(function() {
    clearInterval(skinUpdateInterval);
}, 60000);
...
// Sprawdź i napraw dostęp do skina Leśny Zwiadowca
this.forceUnlockForestSkin();


// Dodaj funkcję do globalnego zakresu
window.forceUpdateSkinImages = forceUpdateSkinImages;
