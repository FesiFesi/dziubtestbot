...
// DODAJ nową funkcję bezpośredniej aktualizacji UI:
    ...
    gameState.missions.forEach(mission => {
        switch(mission.type) {
            case 'collectSeeds':
                ...
                mission.target = seedsTarget; // Aktualizuj również cel misji
                break;
            case 'feedBirds':
                const feedTarget = 2 + Math.floor(playerLevel/2);
                mission.title = t('missions.feedBirds', {amount: feedTarget});
                mission.target = feedTarget;
                break;
            case 'scareBirds':
                const scareTarget = 3 + Math.floor(playerLevel/2);
                mission.title = t('missions.scareBirds', {amount: scareTarget});
                mission.target = scareTarget;
                break;
            case 'autoCollect':
                const autoTarget = 5 + playerLevel;
    ...
    gameState.weeklyMissions.forEach(mission => {
        switch(mission.type) {
            case 'unlockSlots':
                ...
                break;
            case 'feedEpicBirds':
                const epicBirdsAmount = 3 + Math.floor(playerLevel/2);
            ...
            case 'collectCurrency':
                const currencyAmount = 50 + playerLevel * 5;
