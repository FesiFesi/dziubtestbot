...
// Pokaż powiadomienie o wyniku - użyj funkcji tłumaczącej
let locationNameKey = 'locations.parkName'; // domyślnie

if (locationId === 'lake') {
    locationNameKey = 'locations.lakeName';
} else if (locationId === 'forest') {
    locationNameKey = 'locations.forestName';
}

const locationName = t(locationNameKey);
...
let bonusText = "";
if (feedTimeBonus > 0) {
    bonusText = t('locationActions.notifications.feedingTimeBonus', { bonus: feedTimeBonus });
}

if (successfulStandardFeeds > 0 && successfulMythicalFeeds > 0) {
    notificationText = t('locationActions.notifications.fedBirdsAndMythical', { 
    ...
    notificationText = t('locationActions.notifications.fedBirds', { 
        birds: successfulStandardFeeds, 
        location: locationName, 
        seeds: seedsRequired,
        bonus: bonusText
    ...
    notificationText = t('locationActions.notifications.fedMythical', { 
        mythical: successfulMythicalFeeds, 
        location: locationName, 
        fruits: fruitsRequired,
        bonus: bonusText
    ...
    notificationText = t('locationActions.notifications.noBirdsToFeed', { location: locationName });
}
...
function collectAllRewardsInLocation(locationId) {
   ...
   if (successfulCollects > 0) {
    // Użyj funkcji tłumaczącej do pobierania nazwy lokacji
    let locationNameKey = 'locations.parkName'; // domyślnie
...
const locationName = t(locationNameKey);
showNotification(t('locationActions.notifications.noBirdsToFeed', { location: locationName }));
    }
}
