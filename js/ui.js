  ...
  if (hatchButton) {
      ...
      newHatchButton.addEventListener('click', function() {
          ...
          if (typeof window.buyNewEgg === 'function') {
              ...
              window.showNotification("Błąd: nie można kupić jajka");
          }
      });
  } else {
...
function updateBirdSlotUI(slotElement, slotIndex, locationId) {
  ...
  if (!gameState.locationUnlockedSlots[locationId][slotIndex]) {
    ...
    if (unlockButton) {
      ...
      unlockButton.textContent = `Odblokuj (${unlockCost})`;
    }
    if (unlockText) {
      unlockText.style.display = 'block';
      // Aktualizuj tekst kosztu odblokowania
      const unlockCost = locationConfig.slotUnlockCosts[slotIndex] || 50;
      unlockText.textContent = `Odblokuj za ${unlockCost} DziubCoinów`;
    }
    return;
  }
  ...
  if (slot.isActive) {
    ...
    if (slot.needsCollection) {
      ...
      if (feedButton) {
        ...
        if (slot.birdType === 'mythical') {
            ...
            feedButton.textContent = `Nakarm (${fruitCost} 🍎)`;
            ...
            feedButton.textContent = `Nakarm (${cost})`;
        }
    }
    ...
    if (birdTimer) {
      birdTimer.textContent = 'Czekam na ptaka...';
  ...
  updatedFeedButton.addEventListener('click', function() {
    ...
    if (slotData && slotData.isActive && slotData.birdType === 'mythical') {
      ...
      if (gameState.resources.fruits >= fruitCost) {
          ...
          showNotification(`Za mało owoców! Potrzebujesz ${fruitCost} owoców aby nakarmić mitycznego ptaka.`);
      }
    } else {
...
function createMissionElement(mission, index, isWeekly) {
  ...
  missionDiv.setAttribute('data-mission-type', mission.type);
  ...
  // Wybierz ikonę na podstawie typu misji
  let missionIcon = '🎯';
  switch(mission.type) {
    case 'collectSeeds': missionIcon = '🌾'; break;
    case 'feedBirds': missionIcon = '🦜'; break;
    case 'scareBirds': missionIcon = '💨'; break;
    case 'autoCollect': missionIcon = '🔄'; break;
    case 'dailyBonus': missionIcon = '🏆'; break;
    case 'unlockSlots': missionIcon = '🔓'; break;
    case 'feedEpicBirds': missionIcon = '✨'; break;
    case 'collectCurrency': missionIcon = '💰'; break;
    case 'completeDailyMissions': missionIcon = '📋'; break;
  }
  ...
  let html = `
    ...
    <div class="mission-right">
      <div class="mission-reward">
        <img src="assets/images/coin-icon.png" class="coin-icon" alt="DziubCoin">
        <span>${mission.reward}</span>
      </div>
  `;
  
  // Dodaj przycisk lub znacznik ukończenia
  if (mission.completed && !mission.rewarded) {
    html += `<button class="claim-reward-button">Odbierz</button>`;
  } else if (mission.completed && mission.rewarded) {
    html += `<span class="mission-completed">✓ Ukończono</span>`;
  }
  
  html += `</div>`;
  
  // Dodaj strzałkę
  html += `<div class="mission-arrow">›</div>`;
  
  // Dodaj tag NEW dla nowych misji (opcjonalnie)
  if (mission.isNew) {
    html += `<div class="mission-new-tag">NEW</div>`;
  }
  
  missionDiv.innerHTML = html;
  
  // Jeśli przycisk istnieje, dodaj event listener
  if (mission.completed && !mission.rewarded) {
    const claimButton = missionDiv.querySelector('.claim-reward-button');
    if (claimButton) {
...
function updateMissionTimers() {
  ...
  if (dailyTimer) {
    ...
    dailyTimer.textContent = `Resetuje się za ${hoursToReset}h ${minutesToReset}m`;
  }
  
  if (weeklyTimer) {
    weeklyTimer.textContent = "Resetuje się w sobotę o północy UTC";
  }
}
  ...
  // Funkcja dodająca efekt wizualny przy zbieraniu ziarenek (bez wibracji)
  function addSeedCollectEffect(buttonElement) {
    ...
    // Wibracja usunięta zgodnie z prośbą
  }
  ...
  if (collectButton) {
      ...
      newCollectButton.addEventListener('click', function(e) {
          ...
          if (typeof window.addSeedManually === 'function') {
              ...
              alert("Błąd: Funkcja addSeedManually niedostępna!");
          }
      ...
      newCollectButton.setAttribute('onclick', "event.preventDefault(); console.log('Kliknięto przycisk zbierania (onclick)'); if(typeof window.addSeedManually === 'function') {window.addSeedManually();} else if(typeof addSeedManually === 'function') {addSeedManually();} else {console.error('Funkcja addSeedManually niedostępna!'); alert('Błąd: Funkcja addSeedManually niedostępna!');}; return false;");
  ...
  if (collectAutoButton) {
      ...
      newCollectAutoButton.addEventListener('click', function(e) {
          ...
          if (typeof window.collectAutoSeeds === 'function') {
              ...
              alert("Błąd: Funkcja collectAutoSeeds niedostępna!");
          }
...
window.autoCollect = function() {
  ...
  // Kropka powiadamiająca o nagrodzie do odebrania (zaktualizowana)
  ...
  function updateUpgradeButtons() {
    ...
    if (capacityButton) {
      ...
      capacityButton.textContent = `Ulepsz (${cost} DziubCoinów)`;
    }
    ...
    if (speedButton) {
      ...
      speedButton.textContent = `Ulepsz (${cost} DziubCoinów)`;
    }
  }
...
function setupCoinsToFruitsConversion() {
  ...
  convertCoinsButton.onclick = function() {
      ...
      // Sprawdź poprawność
      if (coins <= 0) {
          showNotification("Wprowadź poprawną wartość!");
          return false;
      }
      
      if (coins > gameState.resources.coins) {
          showNotification("Nie masz wystarczającej ilości DziubCoinów!");
          return false;
      }
      ...
      // Pokaż komunikat
      showNotification(`Zamieniono ${coins} DziubCoinów na ${formattedFruits} owoców!`);
      
      // Pokaż animację
   showNotification(`Otrzymałeś ${fruits} owoców za ${coins} DziubCoinów`);


   // Pokaż animację nagrody
const convertButton = document.getElementById('convert-coins-button');
if (convertButton) {
    showRewardAnimation(`+${fruits} owoców`, convertButton);
}
...
function setupFruitsToCoinsConversion() {
  ...
  convertFruitsButton.onclick = function() {
      ...
      if (fruits > gameState.resources.fruits) {
          showNotification("Nie masz wystarczającej ilości owoców!");
          return false;
      }
      ...
      // Pokaż komunikat
      showNotification(`Zamieniono ${formattedFruits} owoców na ${coins} DziubCoinów!`);
...
function setupOtherWalletElements() {
  ...
  if (watchAdButton) {
      watchAdButton.onclick = function() {
          showNotification("Funkcja oglądania reklam będzie dostępna wkrótce!");
          return false;
      };
  }
  ...
  if (withdrawButton) {
      withdrawButton.onclick = function() {
          showNotification("Funkcja wypłaty będzie dostępna wkrótce!");
          return false;
      };
  }
...
setInterval(() => {
  if (document.querySelector('.game-screen.active#missions-screen')) {
    ...
    if (weeklyTimer) {
      ...
      weeklyTimer.textContent = `Resetuje się w sobotę o północy UTC (za ${daysToReset}d ${hoursToReset}h ${minutesToReset}m)`;
    }
  }
}, 30000);
  ...
  // NOWA FUNKCJA - ulepszona aktualizacja UI misji
  function updateMissionsUI() {
    console.log("Aktualizacja UI misji - wersja ulepszona");
    
    // Sprawdź, czy ekran misji jest aktywny
    const missionsScreen = document.getElementById('missions-screen');
    if (!missionsScreen || !missionsScreen.classList.contains('active')) {
      // Jeśli ekran misji nie jest aktywny, nie ma potrzeby aktualizacji
      return;
    }
    
    // Znajdź kontenery misji
    const dailyContainer = document.getElementById('daily-missions-list');
    const weeklyContainer = document.getElementById('weekly-missions-list');
    ...
    // Aktualizuj misje dzienne
    if (gameState.missions && gameState.missions.length > 0) {
      gameState.missions.forEach((mission, index) => {
        updateSingleMissionElement(mission, index, false);
      });
    }
    
    // Aktualizuj misje tygodniowe
    if (gameState.weeklyMissions && gameState.weeklyMissions.length > 0) {
      gameState.weeklyMissions.forEach((mission, index) => {
        updateSingleMissionElement(mission, index, true);
      });
    }
...
// NOWA FUNKCJA: Inteligentna aktualizacja misji
function updateSingleMissionUI(mission, index, isWeekly = false) {
  const selector = isWeekly ? `.weekly-mission-item[data-index="${index}"]` : `.mission-item[data-index="${index}"]`;
  const missionElement = document.querySelector(selector);
  
  if (!missionElement) {
      console.log(`Nie znaleziono elementu misji ${isWeekly ? 'tygodniowej' : 'dziennej'} o indeksie ${index}`);
      // Jeśli element nie istnieje, sprawdź czy kontener istnieje i dodaj element
      const container = isWeekly ? 
        document.getElementById('weekly-missions-list') : 
        document.getElementById('daily-missions-list');
      
      if (container) {
        const newElement = createMissionElement(mission, index, isWeekly);
  ...
  // Aktualizuj klasę misji
  if (mission.completed && mission.rewarded) {
      missionElement.classList.add('mission-rewarded');
      missionElement.classList.remove('mission-complete', 'claimable');
  } else if (mission.completed) {
      missionElement.classList.add('mission-complete', 'claimable');
      missionElement.classList.remove('mission-rewarded');
  } else {
      missionElement.classList.remove('mission-complete', 'claimable', 'mission-rewarded');
  }
  
  // Aktualizuj przycisk odbioru nagrody
  const missionRight = missionElement.querySelector('.mission-right');
  const claimButton = missionElement.querySelector('.claim-reward-button');
  const completedSpan = missionElement.querySelector('.mission-completed');
  
  if (mission.completed && !mission.rewarded) {
      // Misja ukończona, ale nagroda nieodebrana
      if (!claimButton) {
          // Usuń znacznik ukończono jeśli istnieje
          if (completedSpan) completedSpan.remove();
          
          // Utwórz przycisk jeśli nie istnieje
          const newButton = document.createElement('button');
          newButton.className = 'claim-reward-button';
          newButton.textContent = 'Odbierz';
          ...
          if (missionRight) {
              missionRight.appendChild(newButton);
          }
      }
  } else if (mission.completed && mission.rewarded) {
      // Misja ukończona i odebrana
      if (claimButton) claimButton.remove();
      
      if (!completedSpan) {
          const newSpan = document.createElement('span');
          newSpan.className = 'mission-completed';
          newSpan.textContent = '✓ Ukończono';
          if (missionRight) {
              missionRight.appendChild(newSpan);
          }
      }
  } else {
      // Misja nieukończona
...
function updateUpgradeButtonsState() {
  ...
  if (capacityButton) {
      ...
      capacityButton.textContent = `Ulepsz (${capacityCost} DziubCoinów)`;
  ...
  if (speedButton) {
      ...
      speedButton.textContent = `Ulepsz (${speedCost} DziubCoinów)`;
...
function updateSingleMissionUI(mission, index, isWeekly = false) {
  ...
  if (mission.completed && !mission.rewarded) {
      ...
      if (!claimButton) {
          ...
          missionElement.appendChild(newButton);
      }
  } else if (mission.completed && mission.rewarded) {
      // Misja ukończona i odebrana
      if (claimButton) claimButton.remove();
      ...
      if (!completedSpan) {
          ...
          newSpan.className = 'mission-completed';
          newSpan.textContent = '✓ Ukończono';
          missionElement.appendChild(newSpan);
      }
  } else {
      // Misja nieukończona
      if (claimButton) claimButton.remove();
...
window.updateMissionsUI = function() {
  console.log("Inteligentna aktualizacja UI misji");
  
  // ZMIANA: Usunięto sprawdzenie czy ekran misji jest aktywny
  // Teraz zawsze aktualizujemy misje
  
  // Aktualizuj misje dzienne
  if (gameState.missions && gameState.missions.length > 0) {
      gameState.missions.forEach((mission, index) => {
          updateSingleMissionUI(mission, index, false);
      });
  }
  ...
  if (gameState.weeklyMissions && gameState.weeklyMissions.length > 0) {
      gameState.weeklyMissions.forEach((mission, index) => {
          updateSingleMissionUI(mission, index, true);
      });
  }
  ...
  console.log("Zakończono inteligentną aktualizację UI misji");
};
...
// Funkcja pomocnicza do modyfikacji addSeedManually
function enhanceAddSeedManually() {
  const originalAddSeedManually = window.addSeedManually || function() {};
  
  window.addSeedManually = function() {
      const result = originalAddSeedManually.apply(this, arguments);
...
document.addEventListener('DOMContentLoaded', function() {
  // Usuń podwójne wywołanie initProductionScreen
  // setTimeout(initProductionScreen, 500); - TO BYŁO ZDUPLIKOWANE
});
  ...
  window.setupBreedingScreen = function() {
    ...
    if (petBirdImage && petBirdName && petStatus) {
      ...
      if (gameState.petBird && gameState.petBird.exists) {
        ...
        petBirdName.textContent = gameState.petBird.name || 'Pisklak';
        petStatus.textContent = `Twój ${stage === 'egg' ? 'jajko' : 'ptak'} jest w dobrym stanie!`;
      } else {
        // Brak ptaka
        petBirdImage.className = '';
        petBirdName.textContent = 'Brak ptaka';
        petStatus.textContent = 'Zdobądź jajko, aby zacząć hodowlę!';
      }
    }
    ...
    if (hatchButton) {
      hatchButton.onclick = function() {
        ...
        if (typeof window.buyNewEgg === 'function') {
          ...
          showNotification("Funkcja zakupu jajka niedostępna!");
        }
      };
    }
  };
      ...
      newButton.addEventListener('click', function(e) {
          ...
          if (birdType !== 'mythical') {
              ...
              if (finalReward > baseReward) {
                  ...
                  showNotification(`Otrzymałeś ${formatNumber(baseReward, 1)} + ${formatNumber(bonus, 1)} DziubCoinów (bonus z fontanny)!`);
              } else {
                  showNotification(`Otrzymałeś ${formatNumber(finalReward, 1)} DziubCoinów!`);
              }
...
function fixMythicalFeedButtons() {
  ...
  feedButtons.forEach(button => {
      ...
      if (slot.birdType === 'mythical') {
          ...
          newButton.textContent = `Nakarm (${fruitCost} 🍎)`;
              ...
              if (gameState.resources.fruits >= fruitCost) {
                  ...
                  if (gameState.decorations && gameState.decorations[locationId]) {
                      ...
                      if (timeBonus > 0) {
                          ...
                          showNotification(`Karmisz mitycznego ptaka! Bonus -${timeBonus}% do czasu karmienia.`);
                      }
                  }
                  ...
                  showNotification(`Za mało owoców! Potrzebujesz ${fruitCost} owoców.`);
              }
...
document.addEventListener('click', function(e) {
  ...
  // Jeśli kliknięto przycisk prowadzący do ekranu misji
  if (e.target.matches('.nav-button[data-screen="missions-screen"]')) {
      // Wywołaj inteligentną aktualizację misji natychmiast
      updateMissionsUI();
  }
});
...
// NOWA FUNKCJA: Okresowa aktualizacja misji w tle
setInterval(function() {
  // Sprawdź czy ekran misji jest aktywny
  const missionsScreen = document.getElementById('missions-screen');
  if (missionsScreen && missionsScreen.classList.contains('active')) {
      // Wykonaj inteligentną aktualizację UI
      updateMissionsUI();
  }
}, 700); // Co 0,7 sekundy

// NOWA FUNKCJA: Obsługa natychmiastowej aktualizacji po kliknięciu przycisków
const originalUpdateMissionProgress = window.updateMissionProgress || function() {};
window.updateMissionProgress = function(missionType, amount) {
  // Wywołaj oryginalną funkcję
  const result = originalUpdateMissionProgress.apply(this, arguments);
  
  // Natychmiast zaktualizuj UI, jeśli ekran misji jest aktywny
  const missionsScreen = document.getElementById('missions-screen');
  if (missionsScreen && missionsScreen.classList.contains('active')) {
      updateMissionsUI();
  }
...
const originalUpdateWeeklyMissionProgress = window.updateWeeklyMissionProgress || function() {};
window.updateWeeklyMissionProgress = function(missionType, amount) {
  // Wywołaj oryginalną funkcję
  const result = originalUpdateWeeklyMissionProgress.apply(this, arguments);
...
console.log("ui.js załadowany z ulepszoną aktualizacją misji");





// Automatyczne odświeżanie misji co sekundę
setInterval(function() {
  // Zawsze aktualizuj misje
