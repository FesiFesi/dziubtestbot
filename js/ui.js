...
function createMissionElement(mission, index, isWeekly) {
  ...
  let html = `
    ...
    <div class="mission-reward">Nagroda: ${mission.reward} DziubCoinów</div>
  `;
  ...
  if (mission.completed && !mission.rewarded) {
    ...
    claimButton.textContent = 'Odbierz';
    ...
    completedSpan.textContent = '✓ Ukończono';
...
window.autoCollect = function() {
  ...
  // Aktualizacja przycisku bonusu dziennego
  function updateDailyBonusButton() {
    ...
    if (dailyBonusButton) {
      ...
      if (gameState.lastDailyBonus === today) {
        dailyBonusButton.textContent = 'Odebrałeś już dzisiejszy bonus!';
        dailyBonusButton.disabled = true;
      } else {
        dailyBonusButton.textContent = 'Odbierz bonus dzienny!';
...
function setupCoinsToFruitsConversion() {
  ...
  convertCoinsButton.onclick = function() {
      ...
      // Pokaż animację
      showRewardAnimation(`+${formattedFruits} 🍎`, convertCoinsButton);
...
function setupOtherWalletElements() {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
      ...
      // Dodaj obsługę przycisku dziennego bonusu
      const dailyBonusButton = document.getElementById('daily-bonus-button');
      if (dailyBonusButton) {
        dailyBonusButton.onclick = function() {
          console.log("Kliknięto przycisk bonusu dziennego");
          if (typeof claimDailyBonus === 'function') {
            claimDailyBonus();
          } else {
            console.error("Funkcja claimDailyBonus nie jest dostępna!");
          }
        };
      }
    ...
    // Co 5 sekund sprawdzaj, czy można odebrać bonus dzienny
    // (na wypadek, gdyby użytkownik miał otwartą grę przez północ)
    setInterval(updateDailyBonusButton, 5000);
