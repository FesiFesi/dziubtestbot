...
body > #navigation-bar {
    ...
    left: 0 !important;
    width: 100% !important;
    z-index: 1020 !important; /* Wyższy z-index niż ekran ekspedycji */
}
...
<div class="wardrobe-button-locked" style="position: fixed !important; top: 140px !important; right: 15px !important; width: 50px; height: 50px; background-color: #cccccc; border-radius: 50%; display: flex; justify-content: center; align-items: center; box-shadow: 0 3px 10px rgba(0, 0, 0, 0.2); cursor: not-allowed; z-index: 1050 !important; border: 2px solid #999999; opacity: 0.7; pointer-events: auto;">
    ...
    <div style="position: absolute; top: -8px; right: -8px; width: 25px; height: 25px; background-color: #F44336; color: white; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 14px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.3);">🔒</div>
</div>
...
// NIE DODAJEMY już listenera do przycisku szafy - jest zablokowany
// Zamiast tego, dodajemy tooltip z informacją
const wardrobeButtonLocked = expeditionScreen.querySelector('.wardrobe-button-locked');
if (wardrobeButtonLocked) {
    // Dodaj tooltip z informacją, dlaczego przycisk jest zablokowany
    const currentLang = localStorage.getItem('gameLanguage') || 'pl';
    wardrobeButtonLocked.setAttribute('title', currentLang === 'pl' ? 
        'Szafa jest niedostępna podczas aktywnej ekspedycji.' : 
        'Wardrobe is unavailable during active expedition.');
    
    // Dla pewności dodajmy prosty listener, który pokaże powiadomienie
    ...
    showActiveExpeditionScreen: function() {
        ...
        try {
            ...
            if (expeditionScreen) {
                ...
                // Zablokuj scrollowanie dokumentu, aby tło wypełniło cały ekran
...
if (reward.type === 'ton' && reward.amount.includes('-')) {
    // Specjalne losowanie dla TON z dokładnością do 0.001
    ...
    amount = randomMillitons / 1000; // Zamień z powrotem na TON
} else if (reward.amount.includes('-')) {
    // Zakres nagród dla innych typów (np. "500-800")
    const [min, max] = reward.amount.split('-').map(n => parseFloat(n));
...
// DODANE: Sprawdź czy mamy bonus z Leśnego Zwiadowcy
let seedsBonus = 0;
if (window.WardrobeModule && typeof window.WardrobeModule.getExpeditionSeedsBonus === 'function') {
    ...
    if (bonusMultiplier > 0 && collectedRewards.seeds > 0) {
        ...
        console.log(`Dodano bonus za Leśnego Zwiadowcę: +${seedsBonus} nasion (${bonusMultiplier * 100}%)`);
        
        // Dodaj bonus do zebranych nagród
...
if (collectedRewards.seeds > 0) {
    ...
    // Dodaj informację o bonusie, jeśli został zastosowany
    if (seedsBonus > 0) {
        rewardMessage += ` (w tym bonus +${seedsBonus} za Leśnego Zwiadowcę)`;
    }
    rewardMessage += "\n";
}
