// ===== SYSTEM EKSPEDYCJI =====
console.log("Inicjalizacja systemu ekspedycji");

// Główny obiekt zarządzający ekspedycjami
const ExpeditionsModule = {
    // Inicjalizacja modułu
    init: function() {
        console.log("Inicjalizacja modułu ekspedycji");
...
ensureWardrobeButtonWorks: function() {
    console.log("Upewnianie się, że przycisk szafy działa");
    ...
    if (wardrobeButtons.length > 0) {
        wardrobeButtons.forEach(button => {
            ...
            newButton.addEventListener('click', function() {
                console.log("Kliknięto przycisk szafy");
...
#active-expedition-screen {
    ...
    justify-content: center;
    padding: 20px;
    padding-bottom: 80px; /* Dodatkowy padding dla paska nawigacji */
}
...
body > #navigation-bar {
    ...
    /* Dostosuj pozycję kontenera, aby był niżej od przycisku */
    ...
    setupEventListeners: function() {
        console.log("Konfiguracja event listenerów ekspedycji");
        ...
        if (this.activeExpedition) {
            ...
            if (this.activeExpedition.endTime <= Date.now()) {
                ...
                try {
                    ...
                    console.error("Błąd podczas zapisywania stanu oczekujących nagród:", e);
                }
                ...
                showNotification("Ekspedycja zakończona! Odbierz nagrody w zakładce Gniazdo.", 5000);
    ...
    setupExpeditionButton: function(button) {
        ...
        console.log("Ustawiam przycisk ekspedycji:", button);
        ...
        button.onclick = function(event) {
            ...
            if (!expeditionItem) {
                console.error("Nie znaleziono elementu ekspedycji!");
                return;
            }
            
            const expeditionId = expeditionItem.getAttribute('data-expedition-id');
            console.log("Kliknięto przycisk ekspedycji:", expeditionId);
    ...
    updateExpeditionUI: function() {
        console.log("Aktualizacja UI ekspedycji");
        ...
        console.log("Status dostępności ekspedycji:", availabilityStatus);
        ...
        if (availabilityStatus.available) {
            ...
            if (availabilityStatus.reason !== 'level') {
                messageElement.innerHTML += '<p style="font-size: 14px; margin-top: 15px; color: #555;">Wszystkie parametry ptaka muszą być powyżej 75%, aby mógł wyruszyć na ekspedycję.</p>';
                ...
                statsDiv.innerHTML += `
                    <div style="margin-bottom: 15px;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                            <span style="font-weight: bold; color: #333;">Głód:</span>
                ...
                statsDiv.innerHTML += `
                    <div style="margin-bottom: 15px;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                            <span style="font-weight: bold; color: #333;">Szczęście:</span>
                ...
                statsDiv.innerHTML += `
                    <div style="margin-bottom: 5px;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                            <span style="font-weight: bold; color: #333;">Czystość:</span>
                ...
                careButton.textContent = "Zaopiekuj się swoim ptakiem";
    ...
    createExpeditionElement: function(expeditionData) {
        ...
        rewardsTag.textContent = 'NAGRODY';
        ...
        if (expeditionData.cost && expeditionData.cost.type !== 'free') {
            ...
            costBadge.innerHTML = `
                <span style="font-size: 11px; color: #666; margin-right: 4px;">Koszt:</span>
            ...
            freeBadge.innerHTML = `
                <span style="color: #4CAF50; font-weight: bold; font-size: 11px;">✓ BEZPŁATNA</span>
            `;
    ...
    checkAvailability: function() {
        ...
        if (!gameState.petBird || !gameState.petBird.exists || gameState.petBird.level < 4) {
            return {
                available: false,
                reason: "level",
                message: "Twój ptak musi osiągnąć poziom 4, aby móc wyruszyć na ekspedycję."
            };
        }
        ...
        if (gameState.petBird.hunger <= 75) {
            return {
                available: false,
                reason: "hunger",
                message: "Twój ptak jest zbyt głodny! Nakarm go przed wyruszeniem na ekspedycję."
            };
        }
        ...
        if (gameState.petBird.happiness <= 75) {
            return {
                available: false,
                reason: "happiness",
                message: "Twój ptak jest zbyt smutny! Pobaw się z nim przed wyruszeniem na ekspedycję."
            };
        }
        ...
        if (gameState.petBird.cleanliness <= 75) {
            return {
                available: false,
                reason: "cleanliness",
                message: "Twój ptak jest zbyt brudny! Wyczyść go przed wyruszeniem na ekspedycję."
            };
        }
    ...
    startExpedition: function(expeditionId) {
        ...
        try {
            ...
            if (costBadge) {
                ...
                if (costText.includes('BEZPŁATNA')) {
            ...
            if (cost.type === 'fruit' && cost.amount > 0) {
                if (gameState.resources.fruits < cost.amount) {
                    showNotification(`Nie masz wystarczająco owoców! Potrzebujesz ${cost.amount} owoców, aby rozpocząć tę ekspedycję.`);
                    return;
                }
            }
            ...
            if (cost.type === 'fruit' && cost.amount > 0) {
                ...
                showNotification(`Wydano ${cost.amount} owoców na rozpoczęcie ekspedycji.`);
            }
            ...
            // Pokaż powiadomienie
            showNotification(`Rozpoczęto ekspedycję: ${title}!`);
            
        } catch (error) {
            console.error("Błąd podczas rozpoczynania ekspedycji:", error);
            showNotification("Wystąpił błąd podczas rozpoczynania ekspedycji. Spróbuj ponownie.");
        }
    },
    ...
    createActiveExpeditionScreen: function() {
        ...
        try {
            ...
            if (!expedition) {
                console.error("Brak danych aktywnej ekspedycji!");
                return;
            }
            ...
            expeditionScreen.innerHTML = `
                ...
                <button id="expedition-home-button" class="expedition-home-button">← Powrót do Głównej</button>
                <div class="expedition-progress-container">
                    ...
                    <button class="expedition-cancel-button" id="expedition-cancel-button">Anuluj ekspedycję</button>
                </div>
            `;
            ...
            if (rewardButton) {
                rewardButton.onclick = function() {
                    ...
                    if (typeof window.RewardPassModule !== 'undefined' && typeof window.RewardPassModule.showRewardPassScreen === 'function') {
                        ...
                        console.error("Moduł przepustki nagród nie jest dostępny");
                    }
                };
            }
    ...
    createRewardsScreen: function() {
        ...
        rewardsScreen.innerHTML = `
            <div class="expedition-rewards-container">
                <div class="expedition-rewards-title">Ekspedycja zakończona!</div>
                <div class="expedition-rewards-list" id="expedition-rewards-list">
                    <!-- Nagrody będą dodane dynamicznie -->
                </div>
                <button class="collect-rewards-button" id="collect-rewards-button">Odbierz nagrody</button>
            </div>
        `;
    ...
    startExpeditionTimer: function() {
        ...
        if (!this.activeExpedition) {
            console.error("Brak danych aktywnej ekspedycji dla timera!");
            return;
        }
   ...
   // Zakończenie ekspedycji - ZAKTUALIZOWANE
    ...
    prepareRewardsScreen: function() {
        ...
        if (titleElement) {
            titleElement.innerHTML = `<span style="color: #4CAF50;">✓</span> Ekspedycja zakończona!`;
        }
            ...
            if (distanceValue > 0) {
                ...
                distanceBanner.innerHTML = `
                    <div style="font-size: 18px; margin-bottom: 5px; font-weight: bold; color: #9C27B0;">
                        Twój ptak przebył dystans
                    </div>
                    <div style="font-size: 36px; font-weight: bold; color: #7B1FA2; margin: 10px 0;">
                        ${distanceValue} metrów
                    </div>
                    <div style="font-size: 14px; color: #666;">
                        Dystans został dodany do przepustki nagród
                    </div>
                `;
        ...
        this.activeExpedition.rewards.forEach(reward => {
            ...
            if (reward.type === 'seeds') {
                iconUrl = 'assets/images/seed-icon.png';
                rewardLabel = 'ziarenek';
            } else if (reward.type === 'coins') {
                iconUrl = 'assets/images/coin-icon.png';
                rewardLabel = 'DziubCoinów';
            } else if (reward.type === 'ton') {
                iconUrl = 'assets/images/ton-icon.png';
                rewardLabel = 'TON';
            } else if (reward.type === 'fruit') {
            ...
            if (gameState.skins && gameState.skins.currentSkin === 'lesny' && reward.type === 'seeds') {
                ...
                bonusElement.textContent = `(Zawiera +10% bonus z Leśnego Zwiadowcy)`;
            ...
            if (distanceValue > 0) {
                ...
                distanceElement.innerHTML = `
                    ...
                    <div class="expedition-reward-details">
                        ...
                        <div class="expedition-reward-label">dystansu w przepustce</div>
                    </div>
                `;
    ...
    collectRewards: function() {
        ...
        if (hasRewardPass) {
            ...
            if (collectedRewards.distance > 0) {
                ...
                if (collectedRewards.distance >= 10) {
                    ...
                    showNotification(`Twój ptak przebył dystans ${collectedRewards.distance}m! Sprawdź przepustkę nagród.`, 5000);
        ...
        let rewardMessage = "Otrzymano nagrody z ekspedycji:\n";
        if (collectedRewards.seeds > 0) rewardMessage += `- ${collectedRewards.seeds} ziarenek\n`;
        if (collectedRewards.coins > 0) rewardMessage += `- ${collectedRewards.coins} DziubCoinów\n`;
        if (collectedRewards.ton > 0) rewardMessage += `- ${collectedRewards.ton.toFixed(3)} TON\n`;
        if (collectedRewards.fruit > 0) rewardMessage += `- ${collectedRewards.fruit} owoców\n`;
    ...
    // Ukryj odznakę powiadomień
        ...
        // Funkcja resetująca stan szafy
        resetWardrobeState: function() {
            console.log("Resetowanie stanu szafy - ekspedycja zakończona");
            ...
            console.log("Stan szafy zresetowany - przyciski odblokowane");
        },
        ...
        cancelExpedition: function() {
            ...
            if (confirm("Czy na pewno chcesz anulować ekspedycję? Stracisz wszystkie potencjalne nagrody i nie otrzymasz zwrotu kosztów.")) {
                // Ukryj ekran ekspedycji
                const expeditionScreen = document.getElementById('active-expedition-screen');
                if (expeditionScreen) {
                    expeditionScreen.classList.remove('show');
                ...
                // Pokaż powiadomienie
                showNotification("Ekspedycja została anulowana.");
        ...
        checkPendingExpedition: function() {
            ...
            try {
                ...
                if (savedExpedition) {
                    console.log("Znaleziono zapisaną ekspedycję:", savedExpedition);
                    ...
                    if (this.activeExpedition.endTime <= Date.now()) {
                        // Ekspedycja zakończona
                        console.log("Ekspedycja już się zakończyła - ustawiam flagę oczekujących nagród");
                        ...
                        console.log("Ekspedycja w trakcie - kontynuowanie");
                        this.createActiveExpeditionScreen();
                        this.startExpeditionTimer();
                    ...
                    console.log("Brak oczekującej ekspedycji");
