...
(function() {
    ...
    function grayscaleLockedSkins() {
        ...
        skinItems.forEach(skinItem => {
            ...
            // DODANE: Sprawdź, czy to Leśny Zwiadowca
            const isForestScout = title && (title.textContent.includes('Leśny') || 
                                      title.textContent.includes('Forest') || 
                                      title.textContent.includes('zwiadowca') ||
                                      title.textContent.includes('Scout'));
            ...
            // DODANE: Jeśli to Leśny Zwiadowca, sprawdź czy powinien być odblokowany
            if (isForestScout) {
                // Sprawdź czy mamy zapisany dystans w przepustce większy niż 1100m
                let shouldUnlock = false;
                
                // Sprawdź dystans w przepustce
                if (window.gameState && window.gameState.rewardPass && window.gameState.rewardPass.distance >= 1100) {
                    shouldUnlock = true;
                }
                
                // Sprawdź czy jest już odblokowany
                if (window.gameState && window.gameState.skins && 
                ...
                if (window.gameState && window.gameState.rewardPass && 
                    ...
                    window.gameState.rewardPass.claimedRewards.free.includes(1100)) {
                    shouldUnlock = true;
                }
                ...
                if (shouldUnlock) {
                    ...
                    if (window.gameState) {
                        ...
                        } else if (!window.gameState.skins.unlockedSkins.includes("lesny")) {
                    ...
                    button.onclick = function() {
                        if (window.gameState && window.gameState.skins) {
                            window.gameState.skins.currentSkin = "lesny";
                            ...
                            // Zastosuj skin na wszystkich ekranach (jeśli istnieje globalna funkcja)
                            if (typeof window.WardrobeModule !== 'undefined' && 
                                typeof window.WardrobeModule.applySkinToAllScreens === 'function') {
    ...
    style.innerHTML = `
        ...
        /* DODANE: SPECJALNE STYLE DLA LEŚNY ZWIADOWCA - ZAWSZE ODBLOKOWANY PO OSIĄGNIĘCIU 1100m */
        .skin-item[data-skin-id="lesny"] .skin-image img,
        .skin-item:has(.skin-title:contains("Leśny")) .skin-image img,
        .skin-item:has(.skin-title:contains("Forest")) .skin-image img,
        .skin-item:has(.skin-title:contains("zwiadowca")) .skin-image img,
        .skin-item:has(.skin-title:contains("Scout")) .skin-image img {
            filter: none !important;
            opacity: 1 !important;
        }
        
        .skin-item[data-skin-id="lesny"] button,
        .skin-item:has(.skin-title:contains("Leśny")) button,
        .skin-item:has(.skin-title:contains("Forest")) button,
        .skin-item:has(.skin-title:contains("zwiadowca")) button,
        .skin-item:has(.skin-title:contains("Scout")) button {
            background-color: #2196F3 !important;
            cursor: pointer !important;
            opacity: 1 !important;
            pointer-events: auto !important;
