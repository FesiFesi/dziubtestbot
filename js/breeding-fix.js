...
console.log("Inicjalizacja systemu hodowli ptaków");
...
function updatePetBirdUI() {
   ...
   const petLevel = document.getElementById('pet-level-value');
    ...
    if (!gameState.petBird || !gameState.petBird.exists) {
        ...
        if (petBirdName) petBirdName.textContent = "Brak ptaka";
        if (petStatus) petStatus.textContent = "Zdobądź jajko, aby zacząć hodowlę!";
        ...
        if (petLevel) petLevel.textContent = "Poziom: 0";
        ...
        if (hatchButton) {
            ...
            hatchButton.textContent = "Zdobądź jajko (50 💰)";
        }
    ...
    if (petBirdName) {
        petBirdName.textContent = gameState.petBird.name || "Ptak";
    }
    
    if (petStatus) {
        if (gameState.petBird.hunger < 30) {
            petStatus.textContent = `${gameState.petBird.name} jest głodny!`;
        } else if (gameState.petBird.happiness < 30) {
            petStatus.textContent = `${gameState.petBird.name} jest smutny!`;
        } else if (gameState.petBird.cleanliness < 30) {
            petStatus.textContent = `${gameState.petBird.name} potrzebuje czyszczenia!`;
        } else {
            petStatus.textContent = `${gameState.petBird.name} czuje się dobrze!`;
        }
    }
    
    if (petLevel) {
        petLevel.textContent = `Poziom: ${level}`;
    }
    ...
    if (feedButton) {
       ...
       feedButton.innerHTML = `Nakarm (${feedCost} <img src="assets/images/seed-icon.png" style="height: 27px; vertical-align: middle; margin-bottom: 2px;">)`;
...
if (expeditionsButton) {
    if (level >= 4) {
        ...
        if (hungerOK && happinessOK && cleanlinessOK) {
            ...
            expeditionsButton.textContent = "🚀 Ekspedycje";
...
function addPetExperience(amount) {
    ...
    if (gameState.petBird.experience >= experienceNeeded && currentLevel < 4) {
        ...
        if (gameState.petBird.level === 2) {
            ...
            showNotification("Twoje jajko się wykluło! Masz teraz pisklę!");
        } else if (gameState.petBird.level === 3) {
            gameState.petBird.stage = "young";
            showNotification("Twoje pisklę dorosło! Masz teraz młodego ptaka!");
        } else if (gameState.petBird.level === 4) {
            gameState.petBird.stage = "adult";
            showNotification("Twój ptak osiągnął dorosłość!");
            gameState.resources.coins += 100;
            showNotification("Otrzymujesz 100 DziubCoinów za wychowanie dorosłego ptaka!");
        }
...
function feedPet() {
    ...
    if (gameState.petBird.hunger >= 95) {
        showNotification("Ptak jest już najedzony!");
        return;
    }
    ...
    if (gameState.resources.seeds >= feedCost) {
        ...
        addPetExperience(10);
        showNotification(`Nakarmiono ptaka! Zużyto ${feedCost} ziarenek.`);
        updateUI();
        saveGame();
    } else {
        showNotification(`Za mało ziarenek! Potrzebujesz ${feedCost} ziarenek.`);
    }
}
...
function setupPlayScreen() {
    ...
    if (playBackButton) {
        playBackButton.onclick = function() {
            console.log("Powrót do ekranu hodowli");
            // Ukryj ekran zabawy
...
if (playBirdImage) {
    playBirdImage.onclick = function() {
        ...
        console.log("Kliknięto ptaka - zwiększam szczęście!");
        ...
        if (happinessText) {
            happinessText.textContent = `Szczęście: ${Math.round(happiness)}%`;
        }
        ...
        // Pokaż powiadomienie
        showNotification(`Zwiększyłeś szczęście ptaka do ${Math.round(happiness)}%!`);
...
function playWithPet() {
    ...
    if (gameState.petBird.level < 1) {  // Zabawa dostępna od poziomu 1
        showNotification("Ptak jest jeszcze za mały do zabawy!");
        return;
    }
    
    console.log("Uruchamiam ekran zabawy");
    ...
    if (playBirdImage) {
        ...
        console.log(`Ustawiono obrazek ptaka: ./assets/images/bird-${stage}.png`);
    } else {
    ...
    if (happinessText) {
        happinessText.textContent = `Szczęście: ${Math.round(gameState.petBird.happiness)}%`;
    } else {
...
function cleanPet() {
    ...
    addPetExperience(5);
    showNotification(`Wyczyściłeś ${gameState.petBird.name}!`);
    updatePetBirdUI();
    updateUI();
    saveGame();
}
...
function getNewEgg() {
    if (gameState.petBird && gameState.petBird.exists) {
        showNotification("Już masz swojego ptaka!");
        return;
    }
    ...
    if (gameState.resources.coins >= eggCost) {
        ...
        const prefixes = ["Puszek", "Ćwirek", "Słonko", "Świergot", "Błysk", "Skoczek", "Lotnik", "Złotko", "Szmaragd", "Burza"];
        const suffixes = ["Piórko", "Dziubek", "Skrzydło", "Śpiewak", "Strzała", "Błękit", "Ptak", "Serce", "Klejnot", "Anioł"];
        ...
        showNotification(`Zdobyłeś nowe jajko! Nazwałeś je ${randomName}!`);
        updatePetBirdUI();
        updateUI();
        saveGame();
    } else {
        showNotification(`Za mało DziubCoinów! Potrzebujesz ${eggCost} monet.`);
    }
}
...
function setupBreedingScreen() {
    console.log("Inicjalizacja ekranu hodowli");
    ...
    if (cleanButton) {
        cleanButton.onclick = function() {
            ...
            if (gameState.petBird.level >= 3) {
                ...
                showNotification("Potrzebujesz wyższego poziomu ptaka aby to zrobić!");
            }
        };
    }
    ...
    updatePetBirdUI();
    console.log("Ekran hodowli zainicjalizowany pomyślnie");
}
